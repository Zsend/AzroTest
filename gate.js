/*
 * Reserve Standard — Stealth Gate v111 launch-ready
 *
...
(function () {
  ...
  function hasStoredUnlock() {
    try {
      if (typeof sessionStorage !== 'undefined' && sessionStorage.getItem(STORAGE_KEY) === '1') return true;
    } catch (e) { /* ignore */ }
    try {
  ...
  // Check access before applying the fixed/hidden pending state. This prevents
  // already-unlocked mobile sessions from briefly locking the document and then
  // feeling like the page randomly refreshed or jumped.
  if (hasStoredUnlock()) {
    docEl.classList.add('rs-gate-unlocked');
    try { if ('scrollRestoration' in history) history.scrollRestoration = 'auto'; } catch (e) { /* ignore */ }
    return;
  }
  ...
  function initGate() {
    ...
    function unlock() {
      ...
      // Keep unlock stable on mobile: set the initial top position, remove the
      // overlay, then calibrate scroll-triggered effects once after the layout
      // is visible. Repeated forced scrolls made iOS feel jumpy.
