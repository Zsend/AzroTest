/*
 * Reserve Standard — Stealth Gate v151 launch-ready
...
(function () {
  ...
  function markOwnerFromUrl() {
    try {
      ...
      } else if (config.ownerDeviceToken && token === config.ownerDeviceToken) {
  ...
  function sendTracker(eventName, details) {
    ...
    try {
      ...
      // GitHub Pages is static, so the customer-ready tracking path is a
      // simple external endpoint. URL-encoded form posts work cleanly with
      // Google Apps Script, Zapier, Make, Formspree, and most webhook tools.
  ...
  function hasStoredUnlock() {
    ...
    try { if (localStorage.getItem(STORAGE_KEY) === '1') return true; } catch (e) { /* ignore */ }
  ...
  function initGate() {
    ...
    form.addEventListener('submit', function (e) {
      ...
      sha256(pw).then(function (hash) {
        if (config.ownerGateHash && hash === config.ownerGateHash) unlock('owner_code');
        else if (hash === expectedHash) unlock('access_code');
        else {
