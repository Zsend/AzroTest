...
PUBLIC_FILES = [
    "index.html", "proof.html", "404.html", "passport.html", "talent.html", "ops.html",
    "admin.html", "methodology.html", "privacy.html", "terms.html",
    "config.js", "manifest.webmanifest", "robots.txt", "sitemap.xml",
    "assets/mark.svg", "assets/site.css", "assets/product.css", "assets/trust.css", "assets/v4.css",
    "assets/home.js", "assets/proof.js", "assets/static-api.js", "assets/social-card.png",
...
def main() -> int:
    ...
    if not (ROOT / ".github/workflows/pages.yml").exists():
        fail("Missing GitHub Pages deployment workflow", errors)
    ...
    if 'release: "2026.06.19-proof-layer.6"' not in config:
        fail("config.js release identifier is stale", errors)
    if not re.search(r'mode\s*:\s*["\']local["\']', config):
        fail("Published test configuration must default to local mode", errors)
    if "emptyDb" not in bridge or "candidates: []" not in bridge or "registry: []" not in bridge or "claims: []" not in bridge:
        fail("Browser test database must initialize without fabricated people, claims, or registry records", errors)
    proof = (ROOT / "proof.html").read_text(encoding="utf-8")
    required_proof_copy = ["Claims are signals", "Outcomes are proof", "Public credit must be earned", "Right of reply"]
    for phrase in required_proof_copy:
        if phrase not in proof:
            fail(f"proof.html missing required proof-layer language: {phrase}", errors)
    workflow = (ROOT / ".github/workflows/pages.yml").read_text(encoding="utf-8")
    if "proof.html" not in workflow:
        fail("GitHub Pages workflow must publish proof.html", errors)
    for action in ["actions/checkout@v7", "actions/configure-pages@v6", "actions/upload-pages-artifact@v5", "actions/deploy-pages@v5"]:
        if action not in workflow:
            fail(f"GitHub Pages workflow is missing current action: {action}", errors)
    if "include-hidden-files: true" not in workflow:
        fail("GitHub Pages workflow must preserve .nojekyll in the uploaded artifact", errors)
    ...
    if node:
        for rel in ["config.js", "config.production.example.js", "assets/home.js", "assets/proof.js", "assets/static-api.js"]:
