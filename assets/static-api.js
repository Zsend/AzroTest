...
(function () {
  ...
  async function localRoute(path, params, init) {
    ...
    if (path === "/api/health" && method === "GET") return response({ status:"ok", service:"justice-grows-static", environment:"local-test", database:"localStorage", admin_enabled:true, time:now() });
  ...
  function injectModeBanner() {
    ...
    style.textContent = ".jg-test-banner{position:relative;z-index:1000;background:#16251d;color:#fff;border-bottom:1px solid rgba(255,255,255,.18);font:700 11px/1.45 Inter,ui-sans-serif,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif}.jg-test-banner__inner{width:min(calc(100% - 28px),1240px);margin:auto;min-height:44px;display:flex;align-items:center;justify-content:center;gap:10px;text-align:center}.jg-test-banner b{color:#c7f56a}.jg-test-banner a{color:#fff;text-decoration:underline;text-underline-offset:3px}.jg-test-banner code{background:rgba(255,255,255,.1);border-radius:5px;padding:2px 5px}.jg-test-pill{display:inline-block;border:1px solid rgba(255,255,255,.28);border-radius:999px;padding:3px 7px;font-size:9px;letter-spacing:.08em;text-transform:uppercase}@media(max-width:700px){.jg-test-banner__inner{padding:9px 0;display:block}.jg-test-pill{margin-right:6px}}";
    document.head.appendChild(style);
    const banner = document.createElement("div");
    banner.className = "jg-test-banner";
    banner.innerHTML = `<div class="jg-test-banner__inner"><span class="jg-test-pill">Browser test mode</span><span><b>Do not enter real sensitive information.</b> Data stays in this browser only. Test operations at <a href="${baseFileUrl("ops.html")}">ops.html</a> with token <code>${ADMIN_TOKEN}</code>.</span></div>`;
  ...
  window.JusticeGrowsTest = Object.freeze({
    ...
    reset() { localStorage.removeItem(KEY); window.location.reload(); },
