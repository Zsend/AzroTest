...
(function () {
  ...
  const KEY = cfg.localStorageKey || "justice_grows_github_pages_v2";
  ...
  function emptyDb() {
    return {
      ...
      created_at: now(),
      updated_at: now(),
      candidates: [], profiles: [], employers: [], jobs: [], cases: [], corrections: [], partners: [], claims: [],
  ...
  function loadDb() {
    try {
      ...
      if (!parsed || ![1,2].includes(parsed.version)) return emptyDb();
      const fresh = emptyDb();
  ...
  function employerProof(db, employer) {
    const jobs = db.jobs.filter(j => j.employer_id === employer.id && j.status === "published" && (!j.expires_at || j.expires_at >= dateOnly()));
    const jobIds = new Set(db.jobs.filter(j => j.employer_id === employer.id).map(j => j.id));
    const matches = db.matches.filter(m => jobIds.has(m.job_id));
    const starts = matches.filter(m => ["started","retained_90","retained_180","retained_365"].includes(m.stage)).length;
    const retained90 = matches.filter(m => ["retained_90","retained_180","retained_365"].includes(m.stage)).length;
    const retained180 = matches.filter(m => ["retained_180","retained_365"].includes(m.stage)).length;
    const retained365 = matches.filter(m => m.stage === "retained_365").length;
    const interviews = matches.filter(m => ["interview","offer","started","retained_90","retained_180","retained_365"].includes(m.stage)).length;
    const advancement = db.outcomes.filter(o => matches.some(m => m.id === o.match_id) && o.confirmed_by_candidate && ["raise","promotion","credential","equity","ownership"].includes(o.event_type)).length;
    let proof_level = "applied";
    if (["verified","proven"].includes(employer.status)) proof_level = "policy_verified";
    if (jobs.length) proof_level = "role_verified";
    if (starts) proof_level = "hire_verified";
    if (retained180) proof_level = "retention_verified";
    if (advancement) proof_level = "mobility_verified";
    return { proof_level, open_roles:jobs.length, interviews, career_starts:starts, retained_90:retained90, retained_180:retained180, retained_365:retained365, advancement_events:advancement };
  }
  ...
  function metrics(db) {
    ...
    const actual = {
      ...
      open_jobs: db.jobs.filter(j => j.status === "published" && (!j.expires_at || j.expires_at >= dateOnly())).length,
      ...
      retained_90: stageAtLeast(["retained_90", "retained_180", "retained_365"]),
      retained_180: stageAtLeast(["retained_180", "retained_365"]),
      retained_365: stageAtLeast(["retained_365"]),
      advancement_events: db.outcomes.filter(o => o.confirmed_by_candidate && ["raise","promotion","credential","equity","ownership"].includes(o.event_type)).length,
      average_starting_hourly_wage: null
    };
    const wages = db.outcomes.filter(o => o.event_type === "started" && o.confirmed_by_candidate && o.salary_period === "hour" && Number.isFinite(Number(o.wage))).map(o => Number(o.wage));
    if (wages.length) actual.average_starting_hourly_wage = Math.round((wages.reduce((a,b)=>a+b,0) / wages.length) * 100) / 100;
    return {
      mode: "local-test",
      updated_at: db.updated_at,
      actual,
      targets_12_month: { verified_registry_records:250, release_monitoring:200, candidate_intakes:500, profiles_submitted:350, public_profiles:100, verified_employers:50, open_jobs:100, matches:350, interviews:200, offers:125, career_starts:100, retained_90:85, retained_180:75, retained_365:50, advancement_events:30 },
      ...
      disclosure: "Local browser-test metrics are computed only from data entered in this browser. Applications, roles, starts, retention, advancement, targets, and registry records remain separate."
    };
  }
  ...
  function adminQueue(db, kind) {
    ...
    if (kind === "claims") return copy(db.claims);
    if (kind === "matches") return db.matches.map(m => {
      const candidate=db.candidates.find(c=>c.id===m.candidate_id)||{};
      const job=db.jobs.find(j=>j.id===m.job_id)||{};
      const employer=db.employers.find(e=>e.id===job.employer_id)||{};
      return Object.assign({},copy(m),{candidate_name:[candidate.first_name,candidate.last_name].filter(Boolean).join(" "),job_title:job.title||null,org_name:employer.org_name||job.submitted_org_name||null,outcomes:db.outcomes.filter(o=>o.match_id===m.id)});
    });
  ...
  async function localRoute(path, params, init) {
    ...
    if (path === "/api/public/employers" && method === "GET") {
      const items = db.employers.filter(e => ["verified","proven"].includes(e.status) && e.public_name_consent).map(e => Object.assign({ org_name:e.org_name, website:e.website, state:e.state, org_type:e.org_type, states_hiring:e.states_hiring, pathways:e.pathways, status:e.status, last_verified_at:e.verified_at || e.updated_at, updated_at:e.updated_at }, employerProof(db,e)));
      return response({ items, count:items.length, updated_at:db.updated_at, disclosure:"Only employers that choose public attribution and complete the relevant verification stage appear. Absence is an unverified state, not an accusation." });
    }
    if (path === "/api/public/claims" && method === "GET") {
      const allowed = new Set(["verified_role","verified_outcome","closed_no_evidence"]);
      const items = db.claims.filter(c => c.public && allowed.has(c.status)).map(c => ({ id:c.id, company_name:c.company_name, claim_url:c.claim_url, claim_type:c.claim_type, finding_status:c.status, evidence_note:c.evidence_note, right_of_reply_status:c.right_of_reply_status, updated_at:c.updated_at }));
      return response({ items, count:items.length, updated_at:db.updated_at, disclosure:"A published finding describes the evidence reviewed at that time. ‘No qualifying evidence located’ is not a finding that a public claim was false. Material conclusions receive a documented right-of-reply process." });
    }
    ...
    if (path === "/api/intake/employer" && method === "POST") {
      const commitments = ["wage_transparency","no_blanket_ban","fair_chance_process","outcome_reporting","candidate_privacy","advancement_commitment","story_rights"];
    ...
    if (path === "/api/intake/claim" && method === "POST") {
      if (body.website_confirm) return response({ status:"received", message:"Received." }, 201);
      if (!body.company_name || !body.claim_url || !body.claim_type || !body.claim_summary || !bool(body.good_faith)) return error("Complete the public source, claim summary, and good-faith acknowledgment.",422);
      if (!/^https:\/\//i.test(String(body.claim_url))) return error("The claim source must be a public HTTPS link.",422);
      const record = { id:id("clm"), company_name:String(body.company_name).trim(), claim_url:String(body.claim_url).trim(), claim_type:body.claim_type, observed_at:body.observed_at||null, claim_summary:String(body.claim_summary).trim(), outcome_evidence:body.outcome_evidence||null, submitter_name:body.submitter_name||null, submitter_email:body.submitter_email||null, status:"research_lead", public:false, evidence_note:null, right_of_reply_status:"not_started", created_at:now(), updated_at:now() };
      db.claims.push(record); audit(db,"public","claim.created","industry_claim",record.id,{company_name:record.company_name,claim_type:record.claim_type}); saveDb(db);
      return response({id:record.id,status:"research_lead",message:"Saved as a private research lead. It creates no public score, conclusion, or allegation."},201);
    }
    ...
    if (path.startsWith("/api/admin/")) {
      ...
      if((match=path.match(/^\/api\/admin\/employers\/([^/]+)\/verify$/))&&method==="POST"){
        const employer=db.employers.find(e=>e.id===match[1]);if(!employer)return error("Employer not found.",404);employer.status=body.status;employer.verification_notes=body.note||null;if(["verified","proven"].includes(body.status))employer.verified_at=now();employer.updated_at=now();audit(db,"admin","employer.status","employer",employer.id,{status:employer.status});saveDb(db);return response({id:employer.id,status:employer.status});
      }
      ...
      if((match=path.match(/^\/api\/admin\/matches\/([^/]+)\/outcomes$/))&&method==="POST"){
        const item=db.matches.find(x=>x.id===match[1]);if(!item)return error("Match not found.",404);
        const allowed=["started","retained_30","retained_90","retained_180","retained_365","raise","promotion","credential","equity","ownership","separation"];
        if(!allowed.includes(body.event_type))return error("Unsupported outcome event.",422);
        const publicOutcome=body.event_type!=="separation";
        if(publicOutcome&&!bool(body.confirmed_by_candidate))return error("Candidate confirmation is required for every public career outcome.",422);
        const hasStart=["started","retained_90","retained_180","retained_365"].includes(item.stage)||db.outcomes.some(o=>o.match_id===item.id&&o.event_type==="started"&&o.confirmed_by_candidate);
        if(!["started","separation"].includes(body.event_type)&&!hasStart)return error("Record a candidate-confirmed career start before retention or mobility outcomes.",409);
        const outcome={id:id("out"),match_id:item.id,event_type:body.event_type,occurred_at:body.occurred_at||dateOnly(),wage:numberOrNull(body.wage),salary_period:body.salary_period||null,note:body.note||null,confirmed_by_candidate:bool(body.confirmed_by_candidate),confirmed_by_employer:bool(body.confirmed_by_employer),created_at:now()};
        db.outcomes.push(outcome);
        const stageByOutcome={started:"started",retained_30:"started",retained_90:"retained_90",retained_180:"retained_180",retained_365:"retained_365"};if(stageByOutcome[body.event_type])item.stage=stageByOutcome[body.event_type];item.updated_at=now();
        audit(db,"admin","match.outcome","match",item.id,{event_type:outcome.event_type,candidate_confirmed:outcome.confirmed_by_candidate});saveDb(db);return response(Object.assign({},outcome,{stage:item.stage}),201);
      }
      if((match=path.match(/^\/api\/admin\/claims\/([^/]+)\/status$/))&&method==="POST"){
        const item=db.claims.find(x=>x.id===match[1]);if(!item)return error("Industry claim not found.",404);
        item.status=body.status||item.status;item.evidence_note=body.note||item.evidence_note;item.right_of_reply_status=body.right_of_reply_status||item.right_of_reply_status;
        if(bool(body.public)){
          if(!["verified_role","verified_outcome","closed_no_evidence"].includes(item.status))return error("Only a completed evidence disposition may be published.",409);
          if(String(item.evidence_note||"").trim().length<30)return error("A specific evidence note is required before publication.",422);
          if(!["received","declined","expired","not_applicable"].includes(item.right_of_reply_status))return error("Complete the right-of-reply step before publication.",409);
          item.public=true;
        }else item.public=false;
        item.updated_at=now();audit(db,"admin","claim.status","industry_claim",item.id,{status:item.status,public:item.public});saveDb(db);return response({id:item.id,status:item.status,public:item.public});
      }
