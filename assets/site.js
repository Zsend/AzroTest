(function(){
  ...
  function setupRevealTargets(){
    const selectors = [
      '.section-head', '.compact-section-head', '.page-hero .eyebrow', '.page-hero h1', '.page-lead', '.support-line', '.hero .eyebrow', '.hero h1', '.hero .lead', '.hero .support-line', '.hero-photo-frame', '.hero-photo-caption', '.evidence-footer', '.cta-row', '.cta-row .btn',
      ...
      '.thesis-list li', '.compare-list li', '.source-bullets li', '.evidence-chip', '.archive-actions a', '.letter-actions a', '.join', '.join h2', '.join .section-label', '.join p', '.join .side-card', '.join-step', '.form-shell'
    ].join(', ');
  ...
  function setupRevealChoreography(){
    ...
    document.querySelectorAll('.hero').forEach(function(hero){
      var heroOrder = ['.eyebrow','h1','.lead','.support-line','.hero-truths span','.cta-row .btn','.hero-proof .proof-card','.hero-photo-frame','.hero-photo-caption','.evidence-footer','.thesis-card'];
      var n = 0;
      heroOrder.forEach(function(sel){
        hero.querySelectorAll(sel).forEach(function(el){
          if(!el.classList || !el.classList.contains('reveal')) return;
          el.classList.add(n % 3 === 0 ? 'reveal-soft' : (n % 3 === 1 ? 'reveal-rise' : 'reveal-pop'));
          el.style.setProperty('--reveal-delay', Math.min(n, 12) * 72 + 'ms');
          n += 1;
        });
      });
    });
    ...
    document.querySelectorAll('.join').forEach(function(join){
      ...
      ['.section-label','h2','p','.join-step','.form-shell','.side-card','.btn'].forEach(function(sel){
        join.querySelectorAll(sel).forEach(function(el){
          ...
          el.style.setProperty('--reveal-delay', Math.min(n, 14) * 72 + 'ms');
          n += 1;
        });
      });
    });
