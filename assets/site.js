(function(){
  ...
  function setupRevealTargets(){
    const selectors = [
      '.section-head', '.compact-section-head', '.page-hero .eyebrow', '.page-hero h1', '.page-lead', '.support-line', '.cta-row',
      '.card', '.letter-card', '.archive-card', '.minimum-card', '.path-card', '.support-card', '.source-card', '.compare-card', '.proof-card',
      '.panel', '.voice-card', '.room-card', '.hero-side .thesis-card', '.hero-side .photo-card', '.hero-copy', '.letter-collage',
      '.evidence-pill', '.minimum-band', '.logic-band', '.immersive-quote', '.builders-note', '.callout', '.challenge-shell', '.challenge-copy', '.challenge-side', '.challenge-card'
    ].join(', ');
  ...
  function setupRevealChoreography(){
    ...
    all.forEach(function(el){
      ...
      if(el.matches('.hero-copy, .room-copy, .challenge-copy')) el.classList.add('reveal-left');
      if(el.matches('.hero-side, .photo-card, .letter-collage, .challenge-side')) el.classList.add('reveal-right');
      if(el.matches('.section-head, .compact-section-head, .page-hero h1, .page-lead, .support-line')) el.classList.add('reveal-soft');
      if(el.matches('.btn, .cta-row, .evidence-pill')) el.classList.add('reveal-pop');
    });

    const groupSelectors = [
      '.proof-grid', '.support-grid', '.minimum-grid', '.path-grid', '.letter-grid', '.archive-grid', '.sources-grid', '.evidence-grid',
      '.builders-grid', '.public-record-grid', '.vision-grid', '.three-col', '.two-col', '.room-grid', '.compare-grid', '.logic-band', '.hero-truths', '.evidence-strip'
    ];
    ...
    document.querySelectorAll('.hero-grid').forEach(function(group){
      Array.from(group.children).forEach(function(child, index){
        if(child.classList && child.classList.contains('reveal')) child.style.setProperty('--reveal-delay', index * 90 + 'ms');
      });
    });

    document.querySelectorAll('.challenge-shell').forEach(function(group){
      Array.from(group.children).forEach(function(child, index){
        if(child.classList && child.classList.contains('reveal')) child.style.setProperty('--reveal-delay', index * 110 + 'ms');
      });
    });
  }
