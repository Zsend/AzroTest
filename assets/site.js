(function(){
  ...
  function choreographReveals(){
    ...
    document.querySelectorAll('.hero').forEach(function(hero){
      let i = 0;
      const order = ['.eyebrow','h1','.lead','.support-line','.hero-truths span','.cta-row .btn','.hero-proof .proof-card','.photo-card','.hero-photo-frame','.hero-photo-caption','.evidence-footer','.thesis-card'];
      order.forEach(function(sel){
        hero.querySelectorAll(sel).forEach(function(el){
          ...
          if(el.matches('h1,.lead,.support-line')) el.classList.add('reveal-soft');
          setDelay(el, Math.min(i, 18) * 84);
          i++;
        });
      });
    });
    ...
    document.querySelectorAll('.join').forEach(function(join){
      ...
      ['.section-label','h2','p','.join-step','.form-shell','.side-card','.btn'].forEach(function(sel){
        join.querySelectorAll(sel).forEach(function(el){
          if(!el.classList.contains('reveal')) return;
          if(el.closest('.side-actions')) el.classList.add('reveal-right');
          else el.classList.add('reveal-left');
          if(el.matches('.btn,.join-step strong')) el.classList.add('reveal-pop');
          setDelay(el, Math.min(n, 18) * 92);
          n++;
        });
      });
    });
