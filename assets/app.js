(() => {
  ...
  function renderChart(host) {
    ...
    [["0%", "#C42B5B"], ["52%", "#F1BD4B"], ["100%", "#9BBE7D"]].forEach(([offset, color]) => {
    ...
    areaGradient.appendChild(make("stop", { offset: "0%", "stop-color": "#F1BD4B", "stop-opacity": ".16" }));
    areaGradient.appendChild(make("stop", { offset: "100%", "stop-color": "#9BBE7D", "stop-opacity": "0" }));
    ...
    pts.forEach((point, index) => {
      const pointColor = index === 0 ? "#C42B5B" : index === pts.length - 1 ? "#9BBE7D" : "#F1BD4B";
