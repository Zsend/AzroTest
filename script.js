(function(){
  ...
  const whyNowLastCard = whyNowCards[whyNowCards.length - 1] || null;
  const processLastCard = processCards[processCards.length - 1] || null;
  let whyNowCopyTravel = 0;

  const measureWhyNowCopyTravel = () => {
    if (!whyNowCopy || !whyNowLastCard || window.innerWidth < 1024 || prefersReducedMotion()) {
      whyNowCopyTravel = 0;
      if (whyNowCopy) whyNowCopy.style.setProperty('--copy-max-travel', '0px');
      return;
    }

    const previous = whyNowCopy.style.getPropertyValue('--copy-translate');
    whyNowCopy.style.setProperty('--copy-translate', '0px');
    const copyRect = whyNowCopy.getBoundingClientRect();
    const targetRect = whyNowLastCard.getBoundingClientRect();
    whyNowCopyTravel = Math.max(0, targetRect.bottom - copyRect.bottom);
    whyNowCopy.style.setProperty('--copy-max-travel', `${whyNowCopyTravel.toFixed(2)}px`);
    whyNowCopy.style.setProperty('--copy-translate', previous || '0px');
  };

  if (whyNowCopy) whyNowCopy.style.setProperty('--copy-progress', '0');
  if (whyNowCopy) whyNowCopy.style.setProperty('--copy-translate', '0px');
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      if (section.id === 'why-now') {
        ...
        if (whyNowCopy) {
          ...
          const targetRect = whyNowLastCard ? whyNowLastCard.getBoundingClientRect() : null;
          const startTop = window.innerHeight * 0.84;
          const endTop = targetRect ? (window.innerHeight - 28 - targetRect.height) : startTop;
          const travelProgress = targetRect ? clamp((startTop - targetRect.top) / Math.max(1, startTop - endTop)) : 0;
          const travel = whyNowCopyTravel * travelProgress;
          whyNowCopy.style.setProperty('--copy-translate', `${travel.toFixed(2)}px`);
        }
      ...
      if (section.id === 'process') {
        const processTargetRect = processLastCard ? processLastCard.getBoundingClientRect() : null;
        const lineStart = window.innerHeight * 0.88;
        const lineEnd = window.innerHeight * 0.62;
        const processProgress = processTargetRect ? clamp((lineStart - processTargetRect.top) / Math.max(1, lineStart - lineEnd)) : ramp(progress, 0.04, 0.86);
        const processSweep = processTargetRect ? clamp((lineStart - processTargetRect.top) / Math.max(1, lineStart - window.innerHeight * 0.70)) : ramp(progress, 0.08, 0.78);
