(function(){
  ...
  // v82 customer-ready build — directional card reveals, proof-seam contrast, and footer balance polish.
  ...
  if (slideInBandCards.length) {
    ...
    const updateCards = () => {
      ...
      slideInBandCards.forEach((card, idx) => {
        ...
        // v82: direction-aware reveal. Cards load as the customer scrolls
        // down, then keep their furthest reached state when scrolling back up.
        // This avoids content visually "unloading" on reverse scroll, which
        // feels less premium on a serious institutional site.
        const maxProgress = Math.max(card._rsMaxSlideProgress || 0, progress);
        card._rsMaxSlideProgress = maxProgress;

        const e = easeOutCubic(maxProgress);
        const tx = (1 - e) * 140;
  ...
  if (closingScrollCards.length) {
    ...
    const closingMaxProgress = closingScrollCards.map(() => 0);
    ...
    const updateClosingScrollCards = () => {
      ...
      closingScrollCards.forEach((card, idx) => {
        ...
        // v82: one-way scroll reveal. Preserve the carefully tuned arrival
        // points, but never reverse-hide a card once the customer has scrolled
        // through that part of the narrative.
        const progress = Math.max(closingMaxProgress[idx] || 0, rawProgress);
        closingMaxProgress[idx] = progress;
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      if (section.id === 'framework') {
        ...
        frameworkBands.forEach((card, idx) => {
          ...
          const maxBandProgress = Math.max(card._rsMaxBandProgress || 0, bandProgress);
          card._rsMaxBandProgress = maxBandProgress;
          card.style.setProperty('--band-progress', maxBandProgress.toFixed(3));
        });
      }
