...
(function(){
  ...
  const progressFromLine = (line) => {
    ...
    const height = Math.max(1, window.innerHeight || document.documentElement.clientHeight || 1);
    const y = rect.top + rect.height * 0.5;
    // Match the approved mobile rail timing on every viewport.
    const start = height * 0.88;
    const end = height * 0.56;
    return clamp01((start - y) / Math.max(1, start - end));
  };
  ...
  const paint = () => {
    ...
    targets.forEach(({root,line,fill,beam}) => {
      ...
      if (window.innerWidth >= 761) {
        // Move the highlight across the rail itself on desktop. Percent-based
        // translate() is relative to the beam's own width, which made the
        // desktop beam appear almost stationary on long rails.
        setImportant(beam, 'left', `${-8 + raw * 116}%`);
        setImportant(beam, 'transform', 'translate3d(-50%,0,0) skewX(-18deg)');
      } else {
        setImportant(beam, 'left', '0');
        setImportant(beam, 'transform', `translate3d(${-120 + p * 220}%,0,0) skewX(-18deg)`);
      }
      setImportant(beam, 'opacity', String(0.10 + p * 0.62));
    });
...
/* =====================================================================
   v128 — exact desktop/mobile rail scroll parity lock.
   Fixes the two requested rails by tying desktop to the same visible scroll
   window mobile uses: start as the rail enters the lower viewport, complete
   while the rail is still centered on screen. Runs last and paints only these
   two rails, so the rest of the site remains untouched.
   ===================================================================== */
(function(){
  const reduce = window.matchMedia ? window.matchMedia('(prefers-reduced-motion: reduce)') : null;
  const clamp01 = (value) => Math.max(0, Math.min(1, Number.isFinite(value) ? value : 0));
  ...
  const setImportant = (el, prop, value) => { if (el && el.style) el.style.setProperty(prop, value, 'important'); };
  const setVar = (el, name, value) => { if (el && el.style) el.style.setProperty(name, value); };
  const docTop = (el) => {
    if (!el) return 0;
  ...
  const ensureLongRunRail = () => {
    ...
    if (!root) return null;
    let line = root.querySelector('.outcome-call-rail, .chapter-seam-line');
    if (!line) {
    ...
    let fill = line.querySelector('.outcome-call-rail-fill, .chapter-seam-fill');
    if (!fill) {
      fill = document.createElement('span');
      fill.className = 'outcome-call-rail-fill chapter-seam-fill';
    ...
    fill.classList.add('outcome-call-rail-fill', 'chapter-seam-fill');
    let beam = line.querySelector('.outcome-call-rail-beam, .chapter-seam-beam');
    if (!beam) {
      beam = document.createElement('span');
      beam.className = 'outcome-call-rail-beam chapter-seam-beam';
      beam.setAttribute('aria-hidden', 'true');
    ...
    beam.classList.add('outcome-call-rail-beam', 'chapter-seam-beam');
    return { key:'long-run-answer', root, line, fill, beam, top:0 };
  };
  ...
  const getTargets = () => {
    ...
    if (proofRoot) {
      ...
      if (line && fill && beam) targets.push({ key:'outcome-quality', root:proofRoot, line, fill, beam, top:0 });
    }
    return targets;
  };
  ...
  const refreshTargets = () => {
    targets = getTargets();
    targets.forEach((target) => { target.top = docTop(target.line); });
    paint();
  };

  const railProgress = (target) => {
    if (reduce && reduce.matches) return { raw:1, progress:1 };
    const h = viewportH();
    const y = scrollY();
    // Same timing as the mobile rail behavior the user approved.
    const start = target.top - h * 0.88;
    const end = target.top - h * 0.56;
    const raw = clamp01((y - start) / Math.max(1, end - start));
    return { raw, progress: clamp01(0.14 + raw * 0.86) };
  };

  const paintTarget = (target) => {
    if (!target || !target.root || !target.line || !target.fill || !target.beam) return;
    const { raw, progress } = railProgress(target);
    const p = progress.toFixed(4);
    const focus = clamp01(raw * 1.35).toFixed(4);

    setVar(target.root, '--section-progress', raw.toFixed(4));
    setVar(target.root, '--section-focus', focus);
    setVar(target.root, '--chapter-seam-progress', p);
    setVar(target.root, '--rs-live-seam-progress', p);
    setVar(target.root, '--rs-exact-seam-progress', p);
    setVar(target.root, '--outcome-call-line-progress', p);
    setVar(target.root, '--outcome-call-seam-progress', p);

    setImportant(target.fill, 'transform', `scaleX(${progress})`);
    setImportant(target.fill, 'opacity', String(0.18 + progress * 0.78));

    // Desktop needs the beam to travel across the full rail width. Percent
    // translate alone is relative to the beam's own width and is too subtle on
    // long desktop rails; mobile keeps the already-approved compact transform.
    if (window.innerWidth >= 761) {
      setImportant(target.beam, 'left', `${-8 + raw * 116}%`);
      setImportant(target.beam, 'transform', 'translate3d(-50%,0,0) skewX(-18deg)');
    } else {
      setImportant(target.beam, 'left', '0');
      setImportant(target.beam, 'transform', `translate3d(${-120 + progress * 220}%,0,0) skewX(-18deg)`);
    }
    setImportant(target.beam, 'opacity', String(0.10 + progress * 0.62));
  };
  ...
  const paint = () => {
    ticking = false;
    if (!targets.length) targets = getTargets();
  ...
  const requestPaint = () => {
    ...
    window.requestAnimationFrame(() => {
      // Paint one frame later than the older accumulated rail handlers so this
      // scoped lock is the final computed transform on desktop and mobile.
      window.requestAnimationFrame(paint);
    });
  };
  const scrub = (frames) => {
    liveFrames = Math.max(liveFrames, frames || 10);
    requestPaint();
    ...
    const loop = () => {
      ...
      requestPaint();
      liveFrames -= 1;
      if (liveFrames > 0) window.requestAnimationFrame(loop);
      else scrub.active = false;
    };
    window.requestAnimationFrame(loop);
  };

  window.addEventListener('scroll', () => scrub(12), { passive:true });
  window.addEventListener('resize', () => { refreshTargets(); scrub(18); });
  window.addEventListener('orientationchange', () => { window.setTimeout(refreshTargets, 120); scrub(18); });
  window.addEventListener('load', refreshTargets, { once:true });
  window.addEventListener('pageshow', refreshTargets);
  document.addEventListener('rs:gate-unlocked', refreshTargets);
  if (document.fonts && document.fonts.ready) document.fonts.ready.then(refreshTargets).catch(() => {});
  if (reduce && reduce.addEventListener) reduce.addEventListener('change', () => scrub(4));
  [0,80,220,520,1000,1600].forEach((delay) => window.setTimeout(refreshTargets, delay));
  refreshTargets();
})();
