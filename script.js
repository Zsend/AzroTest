(function(){
  ...
  // ---------------------------------------------------------------------
  // v72 — Closing-card scroll choreography with slower Operator + pause.
  //
  // The closing cards now follow the same direct scroll-scrub language as the
  // Asset / Problem / System cards, but the pair is choreographed as one
  // final decision sequence: Operator Materials enters more deliberately, lands,
  // holds for a short scroll beat, and only then does Next Step begin. This
  // makes the Book a Bitcoin Treasury Review card feel like the obvious final
  // action rather than another simultaneous content tile. No timers, no one-
  // time reveal class, and no layout changes.
  // ---------------------------------------------------------------------
  ...
  if (closingScrollCards.length) {
    ...
    // Desktop/wide-tablet choreography: the cards share one row, so use one
    // row-level timeline. Operator now receives a longer scroll window than
    // the framework cards, then the timeline holds for a short beat before
    // Next Step begins. The final card completes while the CTA is still fully
    // visible in the viewport.
    const SEQUENCE_START_RATIO = 1.12;
    const SEQUENCE_END_RATIO = 0.18;
    const ROW_TIMING = [
      { start: 0.00, end: 0.62, drift: 144, blur: 10, scaleFrom: 0.958 },
      { start: 0.72, end: 1.00, drift: 146, blur: 10, scaleFrom: 0.958 }
    ];

    // Stacked responsive choreography: once the two cards are no longer on the
    // same row, each card uses its own viewport position so the second card is
    // not pre-revealed before the customer reaches it. The later start ratio
    // creates the same perceived pause after Operator has landed.
    const STACKED_TIMING = [
      { startRatio: 1.12, endRatio: 0.52, drift: 96, blur: 10, scaleFrom: 0.962 },
      { startRatio: 0.84, endRatio: 0.44, drift: 92, blur: 10, scaleFrom: 0.962 }
    ];

    const localProgressFromRatio = (top, startRatio, endRatio, viewportH) => {
    ...
    const updateClosingScrollCards = () => {
      ...
      if (!isStacked) {
        const startY = viewportH * SEQUENCE_START_RATIO;
        const endY = viewportH * SEQUENCE_END_RATIO;
        sequenceProgress = clamp01((startY - firstRect.top) / Math.max(1, startY - endY));
      }

      closingScrollCards.forEach((card, idx) => {
        const rect = idx === 0 ? firstRect : card.getBoundingClientRect();
        const timing = isStacked
          ? (STACKED_TIMING[idx] || STACKED_TIMING[STACKED_TIMING.length - 1])
          : (ROW_TIMING[idx] || ROW_TIMING[ROW_TIMING.length - 1]);

        const progress = isStacked
          ? localProgressFromRatio(rect.top, timing.startRatio, timing.endRatio, viewportH)
          : clamp01((sequenceProgress - timing.start) / Math.max(0.001, timing.end - timing.start));
        ...
        card.classList.toggle('is-scroll-paused', !isStacked && idx === 0 && sequenceProgress >= 0.62 && sequenceProgress < 0.72);
