...
/* =====================================================================
   v121 — final proof chart + long-run rail engine.
   This is the only code that owns the proof chart draw and the long-run
   answer rail. Desktop timing is tied to the right-side proof story cards
   because the chart lives in a sticky left panel; mobile timing is tied to
   the chart entering the viewport. No card choreography is touched.
   ===================================================================== */
(function(){
  ...
  const desktopProgressFromProofStory = () => {
    if (!proof) return null;
    const rows = Array.from(proof.querySelectorAll('.proof-table-story tbody tr'));
    if (!rows.length) return null;
    const vh = Math.max(1, window.innerHeight || document.documentElement.clientHeight || 1);
    const first = rows[0].getBoundingClientRect();
    const last = rows[rows.length - 1].getBoundingClientRect();
    const firstTop = first.top + window.scrollY;
    const lastBottom = last.bottom + window.scrollY;

    // Start as the first proof card enters the lower reading field. Finish
    // while the final proof card is still visibly in the reading field.
    const start = firstTop - vh * 0.84;
    const end = lastBottom - vh * 0.64;
    return clamp01((window.scrollY - start) / Math.max(1, end - start));
  };

  const mobileProgressFromChart = (chart) => {
    const vh = Math.max(1, window.innerHeight || document.documentElement.clientHeight || 1);
  ...
  const chartProgress = (chart) => {
    if (window.innerWidth >= 1024) {
      const fromStory = desktopProgressFromProofStory();
      if (fromStory !== null) return fromStory;
    }
    return mobileProgressFromChart(chart);
  };

  const paintChart = (chart, raw, reduced) => {
    const p = reduced ? 1 : smooth(raw);
    const endpoint = reduced ? 1 : smooth((p - 0.86) / 0.14);
    ...
    chart.querySelectorAll('[data-line]').forEach((path) => {
      const len = Number(path.dataset.lineLength || 1) || 1;
    ...
    chart.querySelectorAll('[data-chart-area]').forEach((path) => {
      ...
      path.style.setProperty('--chart-area-opacity', String(0.08 + p * 0.92));
    });
    chart.querySelectorAll('[data-endpoint]').forEach((circle) => {
      const scale = 0.72 + endpoint * 0.28;
  ...
  const paintCall = (call, raw, reduced) => {
    ...
    if (fill) {
      ...
      fill.style.opacity = String(0.16 + p * 0.82);
    }
  };
  ...
  const update = () => {
    ...
    charts.forEach((chart) => paintChart(chart, chartProgress(chart), reduced));
    calls.forEach((call) => paintCall(call, lineProgress(call), reduced));
  };
  const request = () => {
    if (ticking) return;
    ticking = true;
...
/* =====================================================================
   v122 — authoritative proof chart draw + long-run rail parity.
   This deliberately runs after the older accumulated handlers. Desktop
   chart timing is tied to the proof shell/right proof stack scroll window
   because the chart is sticky; mobile keeps the viewport-entry behavior.
   Inline important styles are used only on chart SVG properties so older
   !important CSS cannot flatten the draw state.
   ===================================================================== */
(function(){
  const proof = document.getElementById('proof');
  const charts = Array.from(document.querySelectorAll('#proof .path-chart'));
  ...
  const absBottom = (el) => el.getBoundingClientRect().bottom + window.scrollY;

  const setImportant = (el, prop, value) => {
    if (el && el.style) el.style.setProperty(prop, value, 'important');
  };

  const prepChart = (chart) => {
    const svg = chart.querySelector('svg');
    if (!svg) return;
    svg.querySelectorAll('path[stroke^="url("], path[data-line]').forEach((path) => {
      path.setAttribute('data-line', '');
      let len = Number(path.dataset.v122Length || path.dataset.lineLength || 0);
      if (!len) {
        try { len = Math.max(1, path.getTotalLength()); }
        catch (e) { len = 1; }
      }
      path.dataset.v122Length = String(len);
  ...
  charts.forEach(prepChart);

  const desktopProofProgress = () => {
    if (!proof) return 0;
    const shell = proof.querySelector('.proof-shell') || proof;
    const stack = proof.querySelector('.proof-scroll-stack') || proof.querySelector('.proof-grid') || shell;
    const vh = viewportH();
    const start = absTop(shell) - vh * 0.18;
    let end = absBottom(stack) - vh * 0.70;
    const minWindow = vh * 1.20;
  ...
  const chartProgress = (chart) => {
    if (window.innerWidth >= 1024) return desktopProofProgress();
    return mobileChartProgress(chart);
  };

  const paintChart = (chart, raw, reduced) => {
    const p = reduced ? 1 : clamp01(raw);
    const areaP = clamp01((p - 0.02) / 0.92);
    const endpointP = clamp01((p - 0.90) / 0.10);
    chart.style.setProperty('--chart-progress', p.toFixed(4));
    chart.style.setProperty('--chart-ready', p.toFixed(4));

    chart.querySelectorAll('[data-line]').forEach((path) => {
      const len = Number(path.dataset.v122Length || path.dataset.lineLength || 1) || 1;
      const offset = (1 - p) * len;
      path.style.setProperty('--rs-line-length', String(len));
      path.style.setProperty('--rs-line-offset', String(offset));
      setImportant(path, 'stroke-dasharray', String(len));
    ...
    chart.querySelectorAll('[data-chart-area]').forEach((path) => {
      ...
      const opacity = String(areaP <= 0 ? 0 : (0.10 + areaP * 0.90));
      path.style.setProperty('--chart-area-clip', clip);
  ...
  const ensureRailBeam = (call) => {
    const rail = call.querySelector('.outcome-call-rail');
    if (!rail) return null;
    if (!rail.querySelector('.outcome-call-rail-beam')) {
  ...
  const paintCallRail = (call, raw, reduced) => {
    const p = reduced ? 1 : clamp01(raw);
    const seamP = clamp01(0.14 + p * 3.44);
    call.style.setProperty('--outcome-call-line-progress', p.toFixed(4));
  ...
  const update = () => {
    ...
    const reduced = !!(reduceQuery && reduceQuery.matches);
    charts.forEach((chart) => paintChart(chart, chartProgress(chart), reduced));
    calls.forEach((call) => paintCallRail(call, callProgress(call), reduced));
  };
  const request = () => {
    if (ticking) return;
    ticking = true;
  ...
  if (document.fonts && document.fonts.ready) document.fonts.ready.then(request).catch(() => {});
  if (reduceQuery && reduceQuery.addEventListener) reduceQuery.addEventListener('change', request);
  [0,90,280,720].forEach((delay) => window.setTimeout(request, delay));
  request();
})();
