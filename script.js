(function(){
  ...
  // v86 customer-ready build — refined hero scale, precision section anchoring, replayable downward card reveals, final color polish, and restrained ambient motion.
  ...
  const updateScrollFx = () => {
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      if (section.id === 'process') {
        // v93: paired with the dot-to-dot geometry fix in CSS, the line now
        // starts at card-1's dot the moment the section title hits viewport
        // center (progress ~0.05) and reaches card-4's dot at section
        // progress 0.42 — which lands right before the card row scrolls
        // off the top of the viewport on typical desktop heights
        // (cards leave around section-progress 0.45-0.55 depending on
        // viewport/section height ratio). The longer ramp also gives the
        // user actual scroll time to watch the line fill instead of a
        // hard snap. Each card lights up just before the line crosses it.
        const processProgress = ramp(progress, 0.05, 0.37);
        const processSweep = ramp(progress, 0.05, 0.37);
        ...
        processCards.forEach((card, idx) => {
          const stepProgress = ramp(progress, idx * 0.08 + 0.07, 0.20);
