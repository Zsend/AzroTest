(function(){
  ...
  // v103 customer-ready build — last-known-good scroll engine restored — refined hero scale, precision section anchoring, replayable downward card reveals, final color polish, and production launch fixes.
...
/* v102: after the private gate unlocks, repaint the existing v95 scroll engine
   without replacing or overriding its choreography. */
document.addEventListener('rs:gate-unlocked', function(){
  // Repaint the scroll engine after the gate leaves. Do not keep forcing
  // scrollTop after unlock; on iOS that feels like the page is jumping.
  window.setTimeout(function(){ try { window.dispatchEvent(new Event('scroll')); } catch (e) {} }, 80);
  window.setTimeout(function(){ try { window.dispatchEvent(new Event('resize')); } catch (e) {} }, 220);
});

/* v109: seam-position progress. Runs alongside the stable scroll engine and
   only updates custom properties used by chapter-seam cosmetics. It does not
   touch card motion, process connector math, or closing-card choreography. */
(function(){
  var seams = Array.prototype.slice.call(document.querySelectorAll('.chapter-seam'));
  ...
  function paint(){
    ...
    var start = vh * 0.82;
    var end = vh * 0.42;
    var range = Math.max(1, start - end);
    seams.forEach(function(seam){
      var rect = seam.getBoundingClientRect();
      var progress = clamp01((start - rect.top) / range);
      var center = rect.top + rect.height * 0.5;
      var focus = clamp01(1 - Math.abs(center - vh * 0.48) / (vh * 0.58));
      seam.style.setProperty('--seam-progress', progress.toFixed(3));
      seam.style.setProperty('--seam-focus', focus.toFixed(3));
    });
  }
  function schedule(){
    if (ticking) return;
  ...
  document.addEventListener('rs:gate-unlocked', function(){ window.setTimeout(schedule, 80); });
  schedule();
})();
