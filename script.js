(function(){
  ...
  // v118: mobile stability guard, intentionally non-invasive.
  ...
  const RS_SCROLL_STATE_KEY = 'rs_scroll_state_v118';
  ...
  // v118 customer-ready build — last-known-good scroll engine restored — refined hero scale, precision section anchoring, replayable downward card reveals, final color polish, and production launch fixes.
  ...
  // v118: chart motion is owned by the final chart scrub engine at the end of this file.
...
/* v118: seam and outcome-call progress calibration.
...
/* =====================================================================
   v118 — single authoritative proof-chart + outcome-call scroll engine.

   Purpose:
   - Chart growth is now genuinely scroll-scrubbed on desktop and mobile.
   - Desktop uses the proof story stack's document travel, not the sticky
     chart box, so the chart draws through the same reading window the user
     experiences while the right-side proof cards pass by.
   - The long-run treasury answer line uses the same direct scroll-progress
     behavior on desktop and mobile.
   - This owns only .path-chart SVG drawing and .outcome-call-with-line.
   ===================================================================== */
(function(){
  ...
  const setupChart = (chart) => {
    ...
    // Primary/secondary curve strokes.
    ...
    // Area fills reveal from left to right with the line.
    ...
    // Endpoint dots appear only near the end of the draw.
  ...
  const desktopChartProgress = (chart, vh) => {
    const proof = chart.closest('#proof');
    if (!proof) return null;

    const grid = proof.querySelector('.proof-grid') || proof;
    const stack = proof.querySelector('.proof-scroll-stack') || proof.querySelector('.proof-table-story') || grid;

    const gridTop = pageY(grid);
    const stackBottom = pageY(stack) + (stack.getBoundingClientRect().height || stack.offsetHeight || 0);

    // Start when the proof grid enters the comfortable reading field, not as
    // soon as the sticky left chart exists. This prevents the desktop chart from
    // already being mostly drawn when the user first looks at it.
    const startY = gridTop - vh * 0.50;

    // Finish near the end of the right-side proof story stack, so the line grows
    // while the customer scrolls the evidence — the same intent as mobile.
    const endY = stackBottom - vh * 0.72;

    const minSpan = vh * 1.18;
    const span = Math.max(minSpan, endY - startY);
    return clamp01(((window.scrollY || window.pageYOffset || 0) - startY) / span);
  };
  ...
  const paintChart = (chart, progress, reduced) => {
    ...
    const endpoint = reduced ? 1 : smooth((p - 0.72) / 0.28);
    ...
    chart.querySelectorAll('[data-chart-area]').forEach((path) => {
      ...
      path.style.opacity = String(0.08 + p * 0.92);
    });
  ...
  const outcomeLineProgress = (call, vh) => {
    const y = pageY(call);
    // Use document scroll instead of element rect so desktop and mobile use the
    // same deterministic geometry. Starts as the callout enters the reading
    // field and completes before it leaves.
    const startY = y - vh * 0.82;
    const endY = y - vh * 0.38;
    return clamp01(((window.scrollY || window.pageYOffset || 0) - startY) / Math.max(1, endY - startY));
  };
  ...
  const update = () => {
    ...
    calls.forEach((call) => {
      const p = reduced ? 1 : smooth(outcomeLineProgress(call, vh));
