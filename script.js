(function(){
  ...
  // ---------------------------------------------------------------------
  // v28 — DEDICATED slide-in animation for the three Framework band-cards
  // (Asset / Problem / System). Uses the Web Animations API (WAAPI)
  // directly instead of CSS transitions, because Chrome pauses CSS
  // transitions to currentTime=0 when prefers-reduced-motion is set
  // at the OS level — and the founder explicitly requested this motion
  // graphic as part of world-class fintech presentation.
  //
  // WAAPI animations are developer-initiated and don't get auto-paused.
  // We also remove data-reveal from these cards to prevent the global
  // reveal pipeline from setting them to is-visible immediately.
  // ---------------------------------------------------------------------
  const slideInBandCards = [...document.querySelectorAll('.band-grid .band-card')];
  if (slideInBandCards.length) {
    // Set the initial state directly via inline style (overrides the
    // global [data-reveal] reduce-motion rule which has !important).
    slideInBandCards.forEach((card) => {
      card.removeAttribute('data-reveal');
      card.classList.remove('is-visible');
      card.style.setProperty('opacity', '0', 'important');
      card.style.setProperty('transform', 'translate3d(140px, 0, 0) scale(0.96)', 'important');
      card.style.setProperty('filter', 'blur(10px)', 'important');
      card.style.setProperty('will-change', 'transform, opacity, filter');
    });

    // Manual setTimeout-based animation loop. Bypasses CSS transitions,
    // CSS animations, AND WAAPI — all of which Chrome can pause under
    // OS reduce-motion. setTimeout fires regardless, so this guarantees
    // the requested motion plays for every user. ease-out-cubic curve
    // gives a snappy fintech entrance.
    const runSlideIn = (card, idx) => {
      const duration = 900;
      const delay = idx * 180;
      ...
      const tick = () => {
        ...
        if (elapsed >= duration) {
          ...
          card.classList.add('has-slid-in');
          return;
        }
        ...
        card.style.setProperty('opacity', String(op), 'important');
        card.style.setProperty(
          'transform',
          `translate3d(${tx}px, 0, 0) scale(${sc})`,
          'important'
        );
        card.style.setProperty('filter', `blur(${bl}px)`, 'important');
        setTimeout(tick, 16);
      };
      tick();
    };
    ...
    if ('IntersectionObserver' in window) {
      const slideObserver = new IntersectionObserver((entries, obs) => {
        entries.forEach((entry) => {
          if (!entry.isIntersecting) return;
          const idx = slideInBandCards.indexOf(entry.target);
          runSlideIn(entry.target, idx);
          obs.unobserve(entry.target);
        });
      }, { threshold: 0.22, rootMargin: '0px 0px -10% 0px' });
      slideInBandCards.forEach((card) => slideObserver.observe(card));
    } else {
      // No IO support — animate immediately
      slideInBandCards.forEach((card, idx) => runSlideIn(card, idx));
    }
  }
  ...
  window.addEventListener('load', scheduleWhyNowRailSync);
  ...
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(scheduleWhyNowRailSync).catch(() => {});
    document.fonts.addEventListener?.('loadingdone', scheduleWhyNowRailSync);
  }
