(function(){
  ...
  // v172: mobile scroll-position stability. Do not programmatically restore
  // or correct document scroll on mobile. A previous fallback restored a saved
  // session position after mobile reload/pull-to-refresh; in the real customer
  // flow that could look like the page randomly jumped to an old section. From
  // here forward, scroll-linked effects may repaint, but only user action or
  // intentional in-page navigation may move the document.
  try { if ('scrollRestoration' in window.history) window.history.scrollRestoration = 'manual'; } catch (e) { /* ignore */ }
  window.addEventListener('pageshow', () => {
  ...
  if (reviewForm) {
    ...
    reviewForm.addEventListener('submit', async (event) => {
      ...
      if (!name || !business || !email) {
        setReviewStatus('Please complete your name, business, and email before sending.', 'error');
        return;
      }
      if (!validEmail(email)) {
