(function(){
  ...
  if (slideInBandCards.length) {
    ...
    const SCROLL_STAGGER = 110;

    // Progress = 0 when the card's effective top crosses START_RATIO of
    // viewport height (just entering from the bottom).
    // Progress = 1 when the effective top reaches END_RATIO of viewport
    // height (well into the upper-middle area).
    //
    // v30 tuning: END_RATIO raised from 0.28 → 0.50 so the LAST card
    // (System) reaches 100% while still well in the visible area — not
    // after it's scrolled off the top of the viewport (the v29 bug
    // founder caught on the screen recording). With stagger=110 and
    // viewport=900px, card 2 (System) now completes at rect.top ≈ 220
    // — comfortably in view with the card body still showing.
    //
    // Range = 0.85 - 0.50 = 0.35 × viewport ≈ 315px per card. Total
    // reveal span ≈ 315 + 2×110 = 535px. Slow + deliberate.
    const START_RATIO = 0.85;
    const END_RATIO = 0.50;
