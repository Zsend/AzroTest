(function(){
  ...
  const updateScrollFx = () => {
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      const edgeIn = Math.max(0, Math.min(1, (window.innerHeight - rect.top) / Math.max(1, window.innerHeight * 0.9)));
      const edgeOut = Math.max(0, Math.min(1, rect.bottom / Math.max(1, window.innerHeight * 0.9)));
      const chapterStrength = Math.max(0, Math.min(1, Math.min(edgeIn, edgeOut) * 1.05));
      const surfaceY = ((1 - intensity) * 14 - intensity * 5).toFixed(2);
      const surfaceScale = (0.991 + intensity * 0.009).toFixed(4);
