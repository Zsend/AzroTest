  ...
  roots.forEach((root) => {
    ...
    const controller = {
      ...
      render(modeOverride) {
        ...
        let visual;
        if (modeKey === 'video' && allowPreviewAutoplay) {
          visual = document.createElement('video');
          visual.className = 'media-gallery__preview-video';
          visual.src = current.src;
          if (current.poster) visual.poster = current.poster;
          visual.muted = true;
          ...
          visual.setAttribute('loop', '');
          visual.setAttribute('autoplay', '');
          visual.setAttribute('aria-hidden', 'true');
          visual.setAttribute('tabindex', '-1');
          visual.setAttribute('playsinline', '');
          visual.setAttribute('webkit-playsinline', '');
          const tryInlinePlay = () => {
            const playPromise = visual.play();
            if (playPromise && typeof playPromise.catch === 'function') {
              playPromise.catch(() => {});
            }
          };
          visual.addEventListener('loadeddata', tryInlinePlay, { once: true });
...
/* Final publish pass v64: exact desktop hero alignment for home/products/philosophy */
(function () {
  const DESKTOP_BP = 981;
  const clamp = (value, min, max) => Math.max(min, Math.min(max, value));

  function setShift(element, cssVarName, px) {
    if (!element) return;
    if (!px || Math.abs(px) < 1) {
      element.style.setProperty(cssVarName, '0px');
      return;
    }
    element.style.setProperty(cssVarName, `${Math.round(px)}px`);
  }

  function resetHeroShifts() {
    document.querySelectorAll('.home .hero .hero-media').forEach((el) => {
      el.style.removeProperty('--azro-home-media-shift');
    });
    document.querySelectorAll('.products-page .hero-inner--wide-title > .hero-media, .philosophy-page .hero-inner--wide-title > .hero-media').forEach((el) => {
      el.style.removeProperty('--azro-hero-media-shift');
    });
  }

  function alignHomeHeroMedia() {
    const heroText = document.querySelector('.home .hero .hero-text');
    const heroMedia = document.querySelector('.home .hero .hero-media');
    const heading = heroText && heroText.querySelector('h1');
    const cta = heroText && heroText.querySelector('.hero-cta');
    if (!heroText || !heroMedia || !heading || !cta) return;

    const headingRect = heading.getBoundingClientRect();
    const ctaRect = cta.getBoundingClientRect();
    const mediaRect = heroMedia.getBoundingClientRect();
    const targetCenter = (headingRect.top + ctaRect.bottom) / 2;
    const currentCenter = mediaRect.top + (mediaRect.height / 2);
    const delta = clamp(targetCenter - currentCenter, -72, 72);

    setShift(heroMedia, '--azro-home-media-shift', delta);
  }

  function alignSecondaryHeroMedia(pageClass, surfaceSelector) {
    const heroText = document.querySelector(`${pageClass} .hero-inner--wide-title > .hero-text`);
    const heroMedia = document.querySelector(`${pageClass} .hero-inner--wide-title > .hero-media`);
    const surface = document.querySelector(`${pageClass} ${surfaceSelector}`);
    if (!heroText || !heroMedia || !surface) return;

    const textTop = heroText.getBoundingClientRect().top;
    const surfaceTop = surface.getBoundingClientRect().top;
    const delta = clamp(textTop - surfaceTop, -72, 72);

    setShift(heroMedia, '--azro-hero-media-shift', delta);
  }

  function applyHeroAlignmentPolish() {
    resetHeroShifts();

    if (window.innerWidth < DESKTOP_BP) return;

    alignHomeHeroMedia();
    alignSecondaryHeroMedia('.products-page', '.hero-inner--wide-title > .hero-media .media-gallery');
    alignSecondaryHeroMedia('.philosophy-page', '.hero-inner--wide-title > .hero-media .philosophy-hero-panel');
  }

  let frameId = 0;
  function scheduleHeroAlignmentPolish() {
    if (frameId) cancelAnimationFrame(frameId);
    frameId = requestAnimationFrame(applyHeroAlignmentPolish);
  }

  document.addEventListener('DOMContentLoaded', scheduleHeroAlignmentPolish);
  window.addEventListener('load', scheduleHeroAlignmentPolish);
  window.addEventListener('resize', scheduleHeroAlignmentPolish, { passive: true });

  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(scheduleHeroAlignmentPolish).catch(() => {});
  }

  document.querySelectorAll('[data-gallery-root]').forEach((root) => {
    root.addEventListener('click', scheduleHeroAlignmentPolish, true);
  });
})();
