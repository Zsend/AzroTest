(function(){
  ...
  // ---------------------------------------------------------------------
  // v67 — SCROLL-DRIVEN slide-in animation for the closing Operator
  // Materials / Next Step cards. This mirrors the Asset / Problem / System
  // card behavior: fade from zero opacity, resolve from a right-side drift,
  // reduce blur, and settle scale by scroll position. The two-card row is
  // staggered by scroll position so the materials card lands first and the
  // review card follows.
  // ---------------------------------------------------------------------
  const closingSlideCards = [
    ...document.querySelectorAll('#process .download-grid > .doc-showcase.doc-showcase-no-cover'),
    ...document.querySelectorAll('#process .download-grid > .review-launch-card')
  ];

  if (closingSlideCards.length) {
    closingSlideCards.forEach((card) => {
      // Remove the generic one-time reveal pipeline from these two cards.
      // Their entrance is now controlled by scroll position, matching the
      // Framework cards instead of appearing as a timed fade.
      card.removeAttribute('data-reveal');
      card.classList.remove('is-visible');
      card.classList.add('rs-scroll-slide-card');
      card.style.setProperty('--rs-slide-opacity', '0');
      card.style.setProperty('--rs-slide-x', '0px');
      card.style.setProperty('--rs-slide-scale', '0.96');
      card.style.setProperty('--rs-slide-blur', '10px');
    });

    const easeOutCubicClosing = (t) => 1 - Math.pow(1 - t, 3);

    const updateClosingSlideCards = () => {
      const viewportH = Math.max(1, window.innerHeight || 800);
      const viewportW = Math.max(1, window.innerWidth || 1440);
      const isNarrow = viewportW <= 760;
      const startY = viewportH * (isNarrow ? 1.08 : 1.10);
      const endY = viewportH * (isNarrow ? 0.70 : 0.62);
      const range = Math.max(1, startY - endY);
      const travelX = isNarrow ? Math.max(44, Math.min(72, viewportW * 0.16)) : 140;
      const scrollStagger = isNarrow ? 68 : 95;
      const atPageEnd = window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 2;

      closingSlideCards.forEach((card, idx) => {
        const rect = card.getBoundingClientRect();
        const effectiveTop = rect.top + idx * scrollStagger;
        const raw = atPageEnd ? 1 : (startY - effectiveTop) / range;
        const progress = Math.max(0, Math.min(1, raw));
        const eased = easeOutCubicClosing(progress);
        const tx = (1 - eased) * travelX;
        const scale = 0.96 + eased * 0.04;
        const blur = (1 - eased) * 10;

        card.style.setProperty('--rs-slide-opacity', eased.toFixed(4));
        card.style.setProperty('--rs-slide-x', `${tx.toFixed(2)}px`);
        card.style.setProperty('--rs-slide-scale', scale.toFixed(4));
        card.style.setProperty('--rs-slide-blur', `${blur.toFixed(2)}px`);
        card.classList.toggle('is-landed', progress >= 0.995);
      });
    };

    let closingSlideScheduled = false;
    const scheduleClosingSlideUpdate = () => {
      if (closingSlideScheduled) return;
      closingSlideScheduled = true;
      setTimeout(() => {
        updateClosingSlideCards();
        closingSlideScheduled = false;
      }, 0);
    };

    window.addEventListener('scroll', scheduleClosingSlideUpdate, { passive: true });
    window.addEventListener('resize', scheduleClosingSlideUpdate);
    window.addEventListener('load', scheduleClosingSlideUpdate, { once: true });
    window.addEventListener('pageshow', scheduleClosingSlideUpdate);
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(scheduleClosingSlideUpdate).catch(() => {});
    }
    window.setTimeout(scheduleClosingSlideUpdate, 120);
    updateClosingSlideCards();
  }
