...
(() => {
  ...
  function open(btn) {
    ...
    if (descEl) descEl.textContent = desc;
		...
		if (video && src) {
			const startAt = parseFloat(btn.getAttribute('data-start') || '0');
			video.src = src;
			...
			if (Number.isFinite(startAt) && startAt > 0) {
				video.addEventListener('loadedmetadata', () => {
					try { video.currentTime = Math.min(startAt, Math.max(0, video.duration - 0.2)); } catch (_) {}
				}, { once: true });
			}
  ...
  const guideFolds = Array.from(document.querySelectorAll('.fold-card'));
  if (guideFolds.length) {
    guideFolds.forEach((d) => {
      d.addEventListener('toggle', () => {
        if (!d.open) return;
        guideFolds.forEach((other) => { if (other !== d) other.open = false; });
      });
    });
  }
