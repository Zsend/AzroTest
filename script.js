  ...
  const updateScrollFx = () => {
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      if (section.id === 'process') {
        // v91: line starts almost immediately on entering the section and
        // completes by section-progress 0.22 — well before the cards begin
        // scrolling toward the top of viewport. Each card lights up just
        // before the line crosses it. Previous (0.14, 0.26) completed at
        // 0.40, which was too late on tall pages — by then the cards were
        // already near the top edge of viewport.
        const processProgress = ramp(progress, 0.04, 0.18);
        const processSweep = ramp(progress, 0.04, 0.18);
        ...
        processCards.forEach((card, idx) => {
          const stepProgress = ramp(progress, idx * 0.04 + 0.06, 0.14);
