...
(function(){
  ...
  const paintChart = (chart, raw, reduced) => {
    // v149: the Benchmark Lens chart is one-way per viewport visit. It never
  ...
  const chartVisitProgress = (chart) => {
    /*
      v149: Benchmark Lens chart replay model.
      - The visible chart never undraws/reverses.
      - While any part of the chart is on screen, progress can only increase.
      - Once the chart is fully off screen, the reset happens invisibly.
      - On the next downward visit from above, the chart grows again from zero.
      This avoids scroll-direction edge cases on sticky desktop panels and on
      mobile browser address-bar resize events.
    */
    const rect = chart.getBoundingClientRect();
    const height = vh();
    const visible = rect.bottom > 0 && rect.top < height;
    if (!visible) {
      chart.dataset.rsMaxChartProgress = '0';
      chart.dataset.rsChartWasVisible = '0';
      chart.dataset.rsChartVisit = String((Number(chart.dataset.rsChartVisit || 0) || 0) + 1);
      return 0;
    }
    chart.dataset.rsChartWasVisible = '1';
    return getChartProgress(chart);
  };
  const update = () => {
    ticking = false;
...
// v145 publish-readiness audit: no visual changes; cache/version finalization.
