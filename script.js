(function () {
  ...
  function goToPanel(index, options = {}) {
    ...
    if (state.stageMode) {
      ...
      state.lockUntil = performance.now() + (instant ? 0 : LOCK_MS);

      if (stageTrack) {
        stageTrack.style.transitionDuration = instant ? '0ms' : `${TRANSITION_MS}ms`;
        stageTrack.style.transitionTimingFunction = 'cubic-bezier(.16,.92,.2,1)';
      }
