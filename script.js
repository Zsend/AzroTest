(function(){
  ...
  if (slideInBandCards.length) {
    ...
    const updateCards = () => {
      ...
      slideInBandCards.forEach((card, idx) => {
        ...
        const bl = isMobileSlide ? 0 : (1 - e) * 6;
        ...
        card.style.setProperty('filter', bl ? `blur(${bl.toFixed(2)}px)` : 'none', 'important');
        if (storedProgress >= 1) {
          card.classList.add('rs-motion-complete');
          card.style.removeProperty('will-change');
        } else {
          card.classList.remove('rs-motion-complete');
          card.style.setProperty('will-change', 'transform, opacity');
        }
      });
    };

    // Expose to the primary scroll frame so this page does not run a
    // separate scroll listener for the same animation system.
    window.__rsUpdateBandCards = updateCards;

    // Throttle via a scheduled flag — used for resize/load/font events only.
    let scheduled = false;
  ...
  if (closingScrollCards.length) {
    closingScrollCards.forEach((card) => {
      ...
      card.style.setProperty('filter', isMobileClosingInit ? 'none' : 'blur(8px)', 'important');
    });
    ...
    const paintClosingCard = (card, progress, timing, top) => {
      ...
      const bl = isMobileClosing ? 0 : (1 - e) * Math.min(timing.blur, 6);
      ...
      card.style.setProperty('filter', bl ? `blur(${bl.toFixed(2)}px)` : 'none', 'important');
      const landed = hasArrived || progress >= 0.995;
      card.classList.toggle('has-scroll-landed', landed);
      if (landed) {
        card.classList.add('rs-motion-complete');
        card.style.removeProperty('will-change');
      } else {
    ...
    window.__rsUpdateClosingCards = updateClosingScrollCards;
  ...
  let trackedSections = [...document.querySelectorAll('.section[id]')];
  let ticking = false;
  ...
  const updateScrollFx = () => {
    ticking = false;
    syncRSScrollDirection();
    // Closing cards use their own requestAnimationFrame scheduler. Avoid
    // running that heavier card geometry twice on every scroll frame.
    ...
    // v151: one production scroll frame owns all scroll-linked homepage
    // motion. Legacy card/chart handlers expose paint functions instead of
    // registering their own scroll listeners, which prevents duplicate rAFs
    // and visible competition during mobile scroll.
    if (typeof window.__rsUpdateBandCards === 'function') window.__rsUpdateBandCards();
    if (typeof window.__rsUpdateClosingCards === 'function') window.__rsUpdateClosingCards();
    if (typeof window.__rsFinalInteractionUpdate === 'function') window.__rsFinalInteractionUpdate();
  };
  ...
  window.addEventListener('resize', () => {
    trackedSections = [...document.querySelectorAll('.section[id]')];
    scheduleWhyNowRailSync();
    scheduleProofRailSync();
    requestScrollFx();
  });
...
/* =====================================================================
   v151 — consolidated production interaction engine.
   One requestAnimationFrame scheduler owns Proof story cards, Benchmark Lens
   chart drawing, and editorial rails. This removes accumulated duplicate
   scroll engines so motion stays smooth on GitHub Pages/mobile devices.
   ===================================================================== */
(function(){
  const doc = document.documentElement;
  ...
  const proofRows = Array.from(document.querySelectorAll('#proof .proof-table-story tr'));
  ...
  const prepareChart = (chart) => {
    ...
    chart.dataset.rsMaxChartProgress = chart.dataset.rsMaxChartProgress || '0';
    svg.querySelectorAll('path[stroke^="url("], path[data-line]').forEach((path) => {
      path.setAttribute('data-line', '');
      let len = Number(path.dataset.rsLineLength || path.dataset.rsV123Length || path.dataset.lineLength || 0);
      if (!len) {
        try { len = Math.max(1, path.getTotalLength()); }
        catch (e) { len = 1; }
      }
      path.dataset.rsLineLength = String(len);
  ...
  const rawChartProgress = (chart) => window.innerWidth >= 1024 ? desktopChartProgress(chart) : mobileChartProgress(chart);

  const chartVisitProgress = (chart) => {
    /*
      The chart is a proof-build, not a scrubber.
      - When visible, it can only hold or grow.
      - When it leaves above the viewport after the user has passed it, it
        stays completed invisibly so scrolling back up never shows reversal.
      - It resets only when it is fully below the viewport again, so the next
        downward visit from above replays from the beginning.
    */
    const rect = chart.getBoundingClientRect();
    const height = vh();
    ...
    const candidate = rawChartProgress(chart);
  ...
  const paintChart = (chart, progress) => {
    ...
    chart.querySelectorAll('[data-line]').forEach((path) => {
      const len = Number(path.dataset.rsLineLength || path.dataset.rsV123Length || path.dataset.lineLength || 1) || 1;
  ...
  const ensureOutcomeRail = (call) => {
    ...
    return { root: call, line: rail, fill, beam };
  };

  let rails = [];
  const buildRails = () => {
    const next = [];
    document.querySelectorAll('.chapter-seam').forEach((seam) => {
      const line = seam.querySelector('.chapter-seam-line');
      const fill = seam.querySelector('.chapter-seam-fill');
      const beam = seam.querySelector('.chapter-seam-beam');
      if (line && fill && beam) next.push({ root: seam, line, fill, beam });
    });
    document.querySelectorAll('.outcome-call-with-line').forEach((call) => next.push(ensureOutcomeRail(call)));
    ...
    document.querySelectorAll('.next-step-scroll-rule').forEach((line) => {
      const fill = line.querySelector('.next-step-scroll-rule-fill');
      const beam = line.querySelector('.next-step-scroll-rule-beam');
      if (fill && beam) next.push({ root: line, line, fill, beam });
    });
    rails = next;
  };
  buildRails();
  ...
  const progressFromLine = (line) => {
    ...
    return clamp01((start - centerY) / Math.max(1, start - end));
  };

  const paintRail = ({root, line, fill, beam}) => {
  ...
  const paintProofStory = (reduced) => {
    if (reduced) {
      proofRows.forEach((row) => row.style.setProperty('--proof-card-progress', '1'));
      lensMetrics.forEach((metric) => metric.style.setProperty('--proof-lens-progress', '1'));
      return;
    }
    const height = vh();
    proofRows.forEach((row, idx) => {
      const rect = row.getBoundingClientRect();
      const raw = (height * 0.88 - rect.top) / Math.max(1, height * (0.88 - 0.44));
      row.style.setProperty('--proof-card-progress', clamp01(raw - idx * 0.035).toFixed(3));
    });
    lensMetrics.forEach((metric, idx) => {
      const rect = metric.getBoundingClientRect();
      const raw = (height * 0.92 - rect.top) / Math.max(1, height * (0.92 - 0.54));
      metric.style.setProperty('--proof-lens-progress', clamp01(raw - idx * 0.025).toFixed(3));
    });
  };
  ...
  const update = () => {
    ...
    paintProofStory(reduced);
    charts.forEach((chart) => paintChart(chart, chartVisitProgress(chart)));
    rails.forEach(paintRail);
  };

  window.__rsFinalInteractionUpdate = update;
...
// v151 publish-readiness hardening: consolidated scroll engines, one-way chart replay from above, and cache/version finalization.
