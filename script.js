...
(function(){
  ...
  const progressFromLine = (line) => {
    ...
    // Mobile already had the right feel. Desktop now gets the same visible
    // scroll-scrubbed rail pass, but over a wider viewport window so the
    // sweep is readable on large screens instead of completing too abruptly.
    const start = height * (isDesktop ? 0.92 : 0.88);
    const end = height * (isDesktop ? 0.36 : 0.56);
    return clamp01((start - y) / Math.max(1, start - end));
  };
