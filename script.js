(function(){
  ...
  // v98 customer-ready build — restored approved scroll choreography from v95 while preserving v96 mobile readability, gate, and production fixes.
  ...
  const updateScrollFx = () => {
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      if (section.id === 'process') {
        // v98: preserve the approved desktop dot-to-dot connector, and add
        // a true mobile vertical connector that fills between the numbered
        // step circles as the customer scrolls down the stacked card sequence.
        const gridRect = processGrid ? processGrid.getBoundingClientRect() : rect;
        const isMobileProcess = window.matchMedia && window.matchMedia('(max-width: 760px)').matches;
        let processProgress = 0;
        let processSweep = 0;

        if (isMobileProcess && processGrid && processCards.length > 1) {
          const firstStep = processCards[0].querySelector('.step') || processCards[0];
          const lastStep = processCards[processCards.length - 1].querySelector('.step') || processCards[processCards.length - 1];
          const firstRect = firstStep.getBoundingClientRect();
          const lastRect = lastStep.getBoundingClientRect();
          const firstCenter = firstRect.top + firstRect.height / 2;
          const lastCenter = lastRect.top + lastRect.height / 2;
          const stepSpan = Math.max(1, lastCenter - firstCenter);
          const lineStartY = window.innerHeight * 0.82;
          const lineEndY = window.innerHeight * 0.48;

          // Progress is 0 when the first step circle reaches the start zone,
          // and 1 when the final step circle reaches the finish zone. This
          // makes the mobile line feel like the desktop connector translated
          // into a vertical reading path instead of a decorative page overlay.
          processProgress = clamp((lineStartY - firstCenter) / Math.max(1, lineStartY + stepSpan - lineEndY));
          processSweep = processProgress;

          processCards.forEach((card) => {
            const step = card.querySelector('.step') || card;
            const stepRect = step.getBoundingClientRect();
            const stepCenter = stepRect.top + stepRect.height / 2;
            const stepProgress = clamp((lineStartY - stepCenter) / Math.max(1, lineStartY - lineEndY));
            card.style.setProperty('--process-card-progress', stepProgress.toFixed(3));
          });
        } else {
          // Desktop/wide tablet: drive the connector from the actual process-grid
          // position, not from the full section height. This makes the line
          // reach the final card while the row is still visibly in the reading
          // zone on desktop and wide laptop viewports.
          const lineStartY = window.innerHeight * 0.82;
          ...
          processProgress = geometryProgress;
          processSweep = geometryProgress;
        ...
        if (processGrid) {
          processGrid.style.setProperty('--process-progress', processProgress.toFixed(3));
          processGrid.style.setProperty('--process-sweep', processSweep.toFixed(3));
        }
      }
...
/* =====================================================================
   v99 — authoritative motion recovery layer.
   This is intentionally registered last. Earlier repair passes left several
   overlapping scroll systems in place; this final layer owns the visible
   card/connector motion and repaints after the legacy handlers so desktop
   and mobile behave consistently.
   ===================================================================== */
(function(){
  const doc = document.documentElement;
  const clamp = (v, min = 0, max = 1) => Math.max(min, Math.min(max, v));
  const ramp = (p, start, span) => clamp((p - start) / Math.max(0.001, span));
  const easeOutCubic = (t) => 1 - Math.pow(1 - clamp(t), 3);
  const easeSoft = (t) => 1 - Math.pow(1 - clamp(t), 2.55);
  ...
  const setVar = (el, prop, value) => el.style.setProperty(prop, value);

  const groupedBandGrids = [...document.querySelectorAll('.band-grid')]
    .map((grid) => ({ grid, cards: [...grid.querySelectorAll('.band-card')] }))
    .filter((group) => group.cards.length);

  groupedBandGrids.forEach(({ cards }) => {
    cards.forEach((card) => {
      card.removeAttribute('data-reveal');
      card.classList.remove('is-visible');
      card.style.willChange = 'transform, opacity, filter';
      setVar(card, '--band-progress', '0');
      setImportant(card, 'opacity', '0');
      setImportant(card, 'transform', 'translate3d(138px, 0, 0) scale(0.9600)');
      setImportant(card, 'filter', 'blur(10px)');
      card._rsV99Progress = 0;
    });
  });

  const closingCards = [...document.querySelectorAll('.download-grid [data-closing-scroll-card]')];
  ...
  const processGrid = document.querySelector('#process .process-grid-animated');
  const processCards = processGrid ? [...processGrid.querySelectorAll('[data-process-card], .process-card')] : [];
  const frameworkMatrix = document.querySelector('#framework .matrix-shell-animated');
  const frameworkRows = [...document.querySelectorAll('#framework [data-matrix-row]')];

  const managedProgress = (el, raw, effectiveTop, resetY) => {
    const dir = syncDirection();
    let stored = typeof el._rsV99Progress === 'number' ? el._rsV99Progress : 0;

    // Reset only when the element is safely below the entrance zone while the
    // customer moves upward. This gives repeatable downward entrances without
    // the cheap reverse-unload effect inside the reading window.
    if (dir === 'up' && effectiveTop > resetY) {
      stored = 0;
    } else if (dir === 'down') {
      stored = raw;
    } else {
      stored = Math.max(stored, raw);
    }

    stored = clamp(stored);
    el._rsV99Progress = stored;
    return stored;
  };

  const paintSlideCard = (card, progress, drift = 138, blur = 10, scaleFrom = 0.96) => {
    const eased = easeOutCubic(progress);
    const tx = (1 - eased) * drift;
    const sc = scaleFrom + eased * (1 - scaleFrom);
    const bl = (1 - eased) * blur;
    setImportant(card, 'opacity', eased.toFixed(4));
    setImportant(card, 'transform', `translate3d(${tx.toFixed(2)}px, 0, 0) scale(${sc.toFixed(4)})`);
    setImportant(card, 'filter', `blur(${bl.toFixed(2)}px)`);
  };
  ...
  const layoutTop = (el) => {
    ...
    return top - (window.scrollY || window.pageYOffset || 0);
  };

  const updateFrameworkCards = () => {
    const vh = window.innerHeight || doc.clientHeight || 800;
    const startY = vh * 1.10;
    const endY = vh * 0.62;
    const range = Math.max(1, startY - endY);
    const resetY = startY + 34;
    const stagger = Math.max(74, Math.min(108, vh * 0.105));

    groupedBandGrids.forEach(({ cards }) => {
      cards.forEach((card, idx) => {
        const top = card.getBoundingClientRect().top + idx * stagger;
        const raw = clamp((startY - top) / range);
        const progress = managedProgress(card, raw, top, resetY);
        setVar(card, '--band-progress', progress.toFixed(3));
        paintSlideCard(card, progress, window.innerWidth <= 760 ? 72 : 138, 10, 0.96);
      });
    });
  };

  const updateClosingCards = () => {
    if (!closingCards.length) return;
    const vh = window.innerHeight || doc.clientHeight || 800;
    const header = document.querySelector('.site-header');
    const headerBottom = header ? Math.max(0, header.getBoundingClientRect().bottom || header.offsetHeight || 92) : 92;
    const firstTop = layoutTop(closingCards[0]);
    const secondTop = closingCards[1] ? layoutTop(closingCards[1]) : firstTop;
    const stacked = Math.abs(secondTop - firstTop) > 52 || window.innerWidth <= 900;
    const rowTop = stacked ? null : firstTop;
    const arrivalY = clamp(Math.max(vh * 0.275, headerBottom + clamp(vh * 0.10, 54, 112)), headerBottom + 46, vh * 0.34);
    const resetY = vh * 1.16;

    closingCards.forEach((card, idx) => {
      const top = stacked ? layoutTop(card) : rowTop;
      let startY;
      let endY;
      let drift;
      ...
      if (stacked) {
        endY = clamp(Math.max(vh * 0.30, headerBottom + clamp(vh * 0.11, 52, 100)), headerBottom + 42, vh * 0.43);
        startY = endY + (idx === 0 ? clamp(vh * 0.34, 160, 260) : clamp(vh * 0.29, 140, 230));
        drift = idx === 0 ? 84 : 78;
      } else if (idx === 0) {
        startY = vh * 1.20;
        endY = clamp(arrivalY + clamp(vh * 0.33, 230, 340), vh * 0.60, vh * 0.82);
        drift = 144;
      } else {
        // Operator lands, then there is a small scroll beat, then the CTA card
        // lands right as the row reaches the primary reading position.
        startY = clamp(arrivalY + clamp(vh * 0.26, 160, 260), vh * 0.50, vh * 0.72);
        endY = arrivalY;
        drift = 146;
        ease = (t) => Math.pow(clamp(t), 1.10);
      }

      const raw = clamp((startY - top) / Math.max(1, startY - endY));
      let progress = managedProgress(card, raw, top, resetY);
      if (idx === 1 && top <= endY + 0.75) progress = 1;
      card._rsV99Progress = progress;

      const eased = ease(progress);
      const tx = (1 - eased) * drift;
      const sc = 0.958 + eased * 0.042;
      const bl = (1 - eased) * 10;
      setImportant(card, 'opacity', eased.toFixed(4));
      setImportant(card, 'transform', `translate3d(${tx.toFixed(2)}px, 0, 0) scale(${sc.toFixed(4)})`);
      setImportant(card, 'filter', `blur(${bl.toFixed(2)}px)`);
      card.classList.toggle('has-scroll-landed', progress >= 0.995);
      card.classList.toggle('is-cta-focus-window', idx === 1 && progress > 0.12 && progress < 0.995);
    });
  };

  const updateFrameworkMatrix = () => {
    const section = document.querySelector('#framework');
    if (!section) return;
    const rect = section.getBoundingClientRect();
    const vh = window.innerHeight || doc.clientHeight || 800;
    const progress = clamp((vh * 0.84 - rect.top) / Math.max(1, rect.height * 0.72));
    setVar(section, '--matrix-progress', ramp(progress, 0.02, 0.88).toFixed(3));
    if (frameworkMatrix) setVar(frameworkMatrix, '--matrix-sweep', ramp(progress, 0.06, 0.80).toFixed(3));
    frameworkRows.forEach((row, idx) => {
      setVar(row, '--row-progress', ramp(progress, idx * 0.13 + 0.03, 0.28).toFixed(3));
    });
  };
  ...
  const updateProcess = () => {
    ...
    if (isMobile && processCards.length > 1) {
      const firstStep = processCards[0].querySelector('.step') || processCards[0];
      const lastStep = processCards[processCards.length - 1].querySelector('.step') || processCards[processCards.length - 1];
      const firstRect = firstStep.getBoundingClientRect();
      const lastRect = lastStep.getBoundingClientRect();
      ...
      const endLine = vh * 0.46;
      progress = clamp((startLine - firstCenter) / Math.max(1, startLine + span - endLine));

      processCards.forEach((card) => {
        const step = card.querySelector('.step') || card;
        const r = step.getBoundingClientRect();
        const center = r.top + r.height / 2;
        const cardProgress = clamp((startLine - center) / Math.max(1, startLine - endLine));
        setVar(card, '--process-card-progress', cardProgress.toFixed(3));
      });
    } else {
      const rect = processGrid.getBoundingClientRect();
      const startLine = vh * 0.82;
      const endLine = vh * 0.46;
      progress = clamp((startLine - rect.top) / Math.max(1, startLine - endLine));
      processCards.forEach((card, idx) => {
        setVar(card, '--process-card-progress', ramp(progress, idx * 0.22 - 0.02, 0.24).toFixed(3));
      });
    }

    setVar(processGrid, '--process-progress', progress.toFixed(3));
    setVar(processGrid, '--process-sweep', progress.toFixed(3));
    const section = document.querySelector('#process');
    if (section) setVar(section, '--process-progress', progress.toFixed(3));
  };

  const updateAll = () => {
    syncDirection();
    updateFrameworkMatrix();
  ...
  const schedule = () => {
    ...
    if (!timerPending) {
      ...
      // Legacy scroll handlers in older layers also use requestAnimationFrame
      // and zero-delay timers. Paint a second and third beat after them so this
      // authoritative v99 layer is always the final visible state.
      window.setTimeout(updateAll, 18);
      window.setTimeout(updateAll, 54);
    }
  };

  window.addEventListener('scroll', schedule, { passive: true });
  window.addEventListener('resize', schedule, { passive: true });
  window.addEventListener('orientationchange', () => window.setTimeout(schedule, 80), { passive: true });
  window.addEventListener('pageshow', schedule);
  window.addEventListener('load', schedule, { once: true });
  document.addEventListener('rs:gate-unlocked', () => window.setTimeout(schedule, 60));
