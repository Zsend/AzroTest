...
function updateCTAPad(){
  try{
    ...
    if (el){
      ...
      // If the bar is rendered (not display:none) and has reasonable width, use its height
      if (cs && cs.display !== 'none' && r.width > 120){
        h = Math.ceil(r.height || el.offsetHeight || 0);
      }
    }
  ...
  window.addEventListener('scroll', function(){ updateCTAPad(); });
