(function(){
  ...
  // ---------------------------------------------------------------------
  // v74 — Closing-card scroll choreography with early final landing.
  //
  // The closing cards use the same scroll-scrub language as the Asset /
  // Problem / System cards, but the final pair now lands before the row reaches
  // the upper reading zone. Operator Materials enters slowly and deliberately,
  // holds for a small scroll beat, then Next Step slides in and is fully settled
  // while the customer is still arriving at the section. This avoids the card
  // looking half-loaded once the section is visually framed under the header.
  // ---------------------------------------------------------------------
  ...
  if (closingScrollCards.length) {
    ...
    //   • Operator starts slightly below the viewport and finishes at 66vh.
    //   • The pause is the quiet beat from 66vh to 60vh.
    //   • Next Step runs from 60vh to 42vh, so it is fully loaded by the time
    //     the row reaches the main visual focus area.
    const ROW_MOTION = [
      { startRatio: 1.18, endRatio: 0.66, drift: 144, blur: 10, scaleFrom: 0.958 },
      { startRatio: 0.60, endRatio: 0.42, drift: 146, blur: 10, scaleFrom: 0.958 }
    ];
    ...
    // vertical space between cards supplies the pause on mobile/tablet.
    const STACKED_MOTION = [
      { startRatio: 1.14, endRatio: 0.62, drift: 92, blur: 10, scaleFrom: 0.962 },
      { startRatio: 1.04, endRatio: 0.58, drift: 88, blur: 10, scaleFrom: 0.962 }
    ];
    ...
    const updateClosingScrollCards = () => {
      ...
      closingScrollCards.forEach((card, idx) => {
        ...
        const progress = progressFromTop(top, timing.startRatio, timing.endRatio, viewportH);

        paintClosingCard(card, progress, timing);
        card.classList.toggle('is-scroll-paused', !isStacked && idx === 0 && firstRect.top < viewportH * 0.66 && firstRect.top > viewportH * 0.60);
