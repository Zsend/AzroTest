(function(){
  ...
  // ---------------------------------------------------------------------
  // v69 — Closing-card scroll scrub: Operator Materials / Next Step.
  //
  // These two cards now use the SAME motion model as the Asset / Problem /
  // System cards above. This is deliberately not an IntersectionObserver
  // reveal and not a CSS transition. The visible state is derived directly
  // from scroll position on every scroll/resize tick:
  //   • opacity: 0 → 1
  //   • translateX: 140px → 0
  //   • blur: 10px → 0
  //   • scale: .96 → 1
  //   • scroll stagger: 95px between the two cards
  //
  // Keeping the transform string identical to the framework-card transform
  // is important. Prior versions mixed in a hover CSS variable, which made
  // the behavior feel different and could visually flatten the scroll scrub
  // on some browsers. Hover polish is handled with shadow/border only.
  // ---------------------------------------------------------------------
  ...
  if (closingScrollCards.length) {
    closingScrollCards.forEach((card) => {
      card.removeAttribute('data-reveal');
      card.classList.remove('is-visible', 'rs-scroll-slide-card', 'is-landed', 'has-scroll-landed');
      card.style.setProperty('will-change', 'transform, opacity, filter');
    ...
    const CLOSING_SCROLL_STAGGER = 95;
    ...
    const updateClosingScrollCards = () => {
      ...
      closingScrollCards.forEach((card, idx) => {
        const rect = card.getBoundingClientRect();
        const effectiveTop = rect.top + idx * CLOSING_SCROLL_STAGGER;
        const raw = (startY - effectiveTop) / range;
        const progress = Math.max(0, Math.min(1, raw));
