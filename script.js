(function(){
  ...
  const updateWhyNowRail = () => {
    ...
    whyNowRail.style.removeProperty('--copy-rail-height');
    const railRect = whyNowRail.getBoundingClientRect();
    const targetRect = whyNowTargetCard.getBoundingClientRect();
    const copyRect = whyNowCopy.getBoundingClientRect();
    const exactRailHeight = Math.max(copyRect.height, Math.round(targetRect.bottom - railRect.top + 2));
    whyNowRail.style.setProperty('--copy-rail-height', `${exactRailHeight}px`);
  };
  ...
  const updateScrollFx = () => {
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      if (section.id === 'process') {
        const processLineProgress = ramp(progress, 0.00, 0.36);
        const processSweep = ramp(progress, 0.00, 0.40);
        ...
        processCards.forEach((card, idx) => {
          const stepProgress = ramp(progress, idx * 0.075 + 0.015, 0.16);
  ...
  window.addEventListener('load', updateWhyNowRail);
  updateWhyNowRail();
  requestScrollFx();
})();
