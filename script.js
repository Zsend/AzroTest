    ...
    const controller = {
      ...
      render(modeOverride) {
        ...
        if (modeKey === 'video') {
          ...
          poster.src = current.poster || current.src;
          poster.alt = current.label || this.data.title || 'Preview';
          poster.decoding = 'async';
          poster.loading = allowPreviewAutoplay ? 'eager' : 'lazy';
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      ...
      <div class="modal__dialog modal__dialog--support" role="dialog" aria-modal="true" aria-labelledby="supportModalTitle">
        ...
        <p class="support-modal__lead">Email <strong>support@azrosystems.com</strong> for access issues, setup questions, alert troubleshooting, payment verification, or TradingView username changes. Use Contact for direct support and official update channels.</p>
        <div class="support-modal__rows" aria-label="Support checklist">
          ...
          <div class="support-modal__row"><strong>Alert issues</strong><span>Include the alert dialog screenshot, trigger choice, and alert name when the request involves alerts.</span></div>
        ...
        <div class="modal__actions modal__actions--center support-modal__actions">
          <a class="btn btn--sm btn--ghost" href="contact.html">Open contact</a>
