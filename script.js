...
(() => {
  ...
  const getVideoReadyThreshold = () => (
    window.matchMedia('(max-width: 820px), (pointer: coarse)').matches ? 0.18 : 0.05
  );
  ...
  const renderLightbox = () => {
    ...
    const previousStageNode = lbStage.firstElementChild;
    ...
    if (activeSet === 'video') {
      ...
      const useHiddenWarmStage = !!previousStageNode;
      let warmStage = null;
      if (useHiddenWarmStage) {
        warmStage = document.createElement('div');
        warmStage.style.position = 'absolute';
        warmStage.style.inset = '0';
        warmStage.style.opacity = '0';
        warmStage.style.pointerEvents = 'none';
        warmStage.style.overflow = 'hidden';
        warmStage.setAttribute('aria-hidden', 'true');
        warmStage.appendChild(videoWrap);
        lbStage.appendChild(warmStage);
      } else {
        pauseActiveLightboxVideo();
        lbStage.innerHTML = '';
      ...
      let committed = !useHiddenWarmStage;
      let ready = false;
      let playHasStarted = false;
      const readyThreshold = getVideoReadyThreshold();

      const cleanupWarmStage = () => {
        if (warmStage && warmStage.isConnected) warmStage.remove();
      };

      const finalizeSwap = () => {
        if (committed || renderToken !== activeLightboxRenderToken) return;
        committed = true;
        try { previousVideo?.pause(); } catch (err) {}
        cleanupWarmStage();
        lbStage.innerHTML = '';
        lbStage.appendChild(videoWrap);
        activeLightboxVideo = video;
      ...
      const markVideoReady = () => {
        if (ready || renderToken !== activeLightboxRenderToken) return;
        ready = true;
      ...
      const revealWhenPainted = () => {
        if (ready || !playHasStarted || renderToken !== activeLightboxRenderToken) return;
        ...
        if (typeof video.requestVideoFrameCallback === 'function') {
          video.requestVideoFrameCallback(() => {
            if (!playHasStarted || renderToken !== activeLightboxRenderToken) return;
            revealWhenPainted();
          });
          return;
        }
      ...
      const tryPlay = () => {
        if (renderToken !== activeLightboxRenderToken || document.hidden) return;
      ...
      const handleStaleRender = () => {
        if (renderToken === activeLightboxRenderToken) return false;
