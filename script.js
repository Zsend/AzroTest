(function(){
  ...
  // v168: mobile edge-overscroll / pull-to-refresh guard. Prevent only the
  ...
  const saveRSScrollState = () => {
    try {
      ...
      if (y < 24) {
        sessionStorage.removeItem(RS_SCROLL_STATE_KEY);
        return;
      }
  ...
  window.addEventListener('pageshow', () => {
    // Do not synthesize a scroll event here. Some mobile browsers restore BFCache
    // state asynchronously; forcing every scroll engine during that window can
    // look like a page jump. Individual engines listen to pageshow below.
  }, { once: true });

  // If a mobile browser still performs a true pull-to-refresh/reload, restore the
  // reader to the saved position rather than leaving them unexpectedly at the top.
  // This only runs on real reload/back-forward restores, never on a fresh visit.
  const maybeRestoreMobileReloadScroll = () => {
    if (!rsIsTouchLike() || window.location.hash) return;
    let navType = '';
    try {
    ...
    if (doc.classList.contains('rs-gate-pending') || doc.classList.contains('rs-gate-locked')) return;
    try {
      const raw = sessionStorage.getItem(RS_SCROLL_STATE_KEY);
      if (!raw) return;
      const saved = JSON.parse(raw);
      if (!saved || saved.path !== currentPathForRestore()) return;
      if (!saved.y || saved.y < 240 || Date.now() - Number(saved.t || 0) > 5 * 60 * 1000) return;
      if (getRSScrollY() > 32) return;
      markRSScriptedScroll(900);
      window.requestAnimationFrame(() => window.requestAnimationFrame(() => {
        try { window.scrollTo({ top: Math.max(0, Math.round(saved.y)), left: 0, behavior: 'auto' }); } catch (e) {}
      }));
    } catch (e) { /* ignore */ }
  };
  window.addEventListener('load', () => window.setTimeout(maybeRestoreMobileReloadScroll, 80), { once:true });
  document.addEventListener('rs:gate-unlocked', () => window.setTimeout(maybeRestoreMobileReloadScroll, 80));
  ...
  window.addEventListener('resize', refreshPrimaryScrollFx);
...
(function(){
  ...
  const update = () => {
    ...
    charts.forEach((chart) => paintChart(chart, chartVisitProgress(chart), reduced));
...
(function(){
  ...
  const paint = () => {
    ...
    chapterSeams.forEach((seam) => {
      ...
      seam.style.setProperty('--chapter-seam-progress', seamP.toFixed(4), 'important');
    });

    rails.forEach(({ call, rail, fill, beam }) => {
      const seamP = seamProgressFromLine(rail);
      call.style.setProperty('--outcome-call-line-progress', seamP.toFixed(4), 'important');
      call.style.setProperty('--outcome-call-seam-progress', seamP.toFixed(4), 'important');
...
(function(){
  ...
  const paintRail = ({ owner, rail, fill, beam }, reduced) => {
    const p = progressForRail(rail);
    if (owner) {
      owner.style.setProperty('--chapter-seam-progress', p.toFixed(4), 'important');
      owner.style.setProperty('--outcome-call-seam-progress', p.toFixed(4), 'important');
      owner.style.setProperty('--outcome-call-line-progress', p.toFixed(4), 'important');
      owner.style.setProperty('--rs-exact-seam-progress', p.toFixed(4), 'important');
    }
