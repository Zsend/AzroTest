...
(() => {
  ...
  let videoDownloadEl = modal.querySelector('[data-video-download]');
  if (!videoDownloadEl && videoControls) {
    videoDownloadEl = document.createElement('a');
    videoDownloadEl.className = 'btn btn--sm btn--ghost';
    videoDownloadEl.setAttribute('data-video-download', '');
    videoDownloadEl.textContent = 'Download video';
    videoDownloadEl.href = '#';
    videoDownloadEl.hidden = true;
    videoControls.appendChild(videoDownloadEl);
  }
  ...
  // On-brand interaction: tap video to pause/resume when native controls are not active.
  if (video && !video.dataset.azroTapBound) {
    video.dataset.azroTapBound = '1';
    video.addEventListener('click', () => {
      if (video.controls || currentVideoMode === 'demo') return;
      if (video.paused) {
        video.play().catch(() => {});
      } else {
        video.pause();
  ...
  const getFileNameFromPath = (src = '') => {
    if (!src) return 'video';
    return src.split('/').pop().split('?')[0].split('#')[0] || 'video';
  };
  ...
  function close() {
    ...
    if (videoDownloadEl) {
      videoDownloadEl.hidden = true;
      videoDownloadEl.removeAttribute('href');
      videoDownloadEl.removeAttribute('download');
    }
    if (video) {
  ...
  function open(btn) {
    ...
    if (videoDownloadEl) {
      if (src) {
        videoDownloadEl.href = src;
        videoDownloadEl.setAttribute('download', getFileNameFromPath(src));
        videoDownloadEl.hidden = false;
      } else {
        videoDownloadEl.hidden = true;
        videoDownloadEl.removeAttribute('href');
        videoDownloadEl.removeAttribute('download');
...
(() => {
    ...
    lightbox.innerHTML = `
      ...
      <div class="modal__dialog modal__dialog--gallery" role="dialog" aria-modal="true" aria-label="Preview gallery">
        ...
        <div class="gallery-lightbox__footer">
          ...
          <div class="gallery-lightbox__actions" data-gallery-lightbox-actions hidden>
            <a class="btn btn--sm" data-gallery-lightbox-download href="#" download>Download video</a>
          </div>
  ...
  const lbActions = lightbox.querySelector('[data-gallery-lightbox-actions]');
  const lbDownload = lightbox.querySelector('[data-gallery-lightbox-download]');
  ...
  const renderLightbox = () => {
    ...
    if (activeSet === 'video') {
      if (lbActions) lbActions.hidden = false;
      if (lbDownload) {
        lbDownload.href = current.src;
        lbDownload.setAttribute('download', getFileNameFromPath(current.src));
      }
      ...
      if (lbActions) lbActions.hidden = true;
      if (lbDownload) {
        lbDownload.removeAttribute('href');
        lbDownload.removeAttribute('download');
      }
      ...
      render(modeOverride) {
        ...
        if (modeKey === 'video') {
          ...
          if (allowPreviewAutoplay) {
            ...
            video.setAttribute('playsinline', '');
            video.setAttribute('webkit-playsinline', '');
            const markReady = () => visualWrap.classList.add('is-video-ready');
            const tryInlinePlay = () => {
              const playPromise = video.play();
              if (playPromise && typeof playPromise.catch === 'function') {
                playPromise.catch(() => {});
            ...
            video.addEventListener('error', () => visualWrap.classList.remove('is-video-ready'));
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      ...
      <div class="modal__dialog modal__dialog--support" role="dialog" aria-modal="true" aria-labelledby="supportModalTitle">
        ...
        <p class="support-modal__lead">Email <strong>support@azrosystems.com</strong> for access issues, setup questions, alert troubleshooting, payment verification, or TradingView username changes. Use Support for setup docs, access steps, and official support routes.</p>
        <div class="support-modal__rows" aria-label="Support checklist">
          ...
          <div class="support-modal__row"><strong>Reply window</strong><span>Support replies and invite-only access are typically handled within 24 business hours or less.</span></div>
        </div>
