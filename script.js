(function(){
  ...
  // v121: mobile stability guard, intentionally non-invasive.
  // Earlier builds tried to manually restore scroll position on load/reload;
  // on mobile that could feel like a page jump. Let the browser own scroll
  // restoration, save a cheap breadcrumb only on tab suspension, and merely
  // repaint scroll-linked variables when a page returns from BFCache.
  try { if ('scrollRestoration' in window.history) window.history.scrollRestoration = 'auto'; } catch (e) { /* ignore */ }
  const RS_SCROLL_STATE_KEY = 'rs_scroll_state_v121';
  const currentPathForRestore = () => `${window.location.pathname}${window.location.search}`;
  const saveRSScrollState = () => {
    try {
      const y = Math.round(getRSScrollY());
      if (y < 24) return;
      sessionStorage.setItem(RS_SCROLL_STATE_KEY, JSON.stringify({ path: currentPathForRestore(), y, t: Date.now() }));
    } catch (e) { /* ignore */ }
  };
  window.addEventListener('pagehide', saveRSScrollState, { passive: true });
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') saveRSScrollState();
  });
  window.addEventListener('pageshow', () => {
  ...
  const scheduleProofRailSync = () => {
    window.requestAnimationFrame(() => window.requestAnimationFrame(syncProofRail));
  };
  ...
  const updateScrollFx = () => {
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      if (section.id === 'process') {
        ...
        const geometryProgress = clamp((lineStartY - gridRect.top) / Math.max(1, lineStartY - lineEndY));
        const processProgress = geometryProgress;
        const processSweep = geometryProgress;
        ...
        processCards.forEach((card, idx) => {
          const stepProgress = ramp(processProgress, idx * 0.22 - 0.02, 0.24);
...
document.addEventListener('rs:gate-unlocked', function(){
  window.requestAnimationFrame(function(){
    try {
      ...
      if (typeof scheduleProofRailSync === 'function') scheduleProofRailSync();
    } catch (e) {}
  });
});
...
(function(){
  ...
  const update = () => {
    ...
    rows.forEach((row, idx) => {
      ...
      setProgress(row, '--proof-card-progress', clamp(raw - stagger));
    });
    lensMetrics.forEach((metric, idx) => {
      ...
      const end = vh * 0.54;
      const raw = (start - rect.top) / Math.max(1, start - end);
      setProgress(metric, '--proof-lens-progress', clamp(raw - idx * 0.025));
    });
  };
  const request = () => {
    if (ticking) return;
...
(function(){
  ...
  const desktopChartProgress = (chart) => {
    ...
    const stack = proof ? (proof.querySelector('.proof-scroll-stack') || proof.querySelector('.proof-grid') || proof) : null;
    const start = chartTop - height * 0.78;
    let end = start + height * 1.78;
    if (stack) {
      const stackLimit = docBottom(stack) - height * 0.34;
      if (stackLimit > start + height * 1.08) end = Math.min(end, stackLimit);
    }
    if (end <= start) end = start + height * 1.55;
  ...
  const paintChart = (chart, raw, reduced) => {
    ...
    chart.style.setProperty('--chart-progress', p.toFixed(4));
    chart.style.setProperty('--chart-ready', p.toFixed(4));

    chart.querySelectorAll('[data-line]').forEach((path) => {
    ...
    chart.querySelectorAll('[data-chart-area]').forEach((path) => {
    ...
    chart.querySelectorAll('[data-endpoint]').forEach((circle) => {
  ...
  const update = () => {
    ...
    charts.forEach((chart) => paintChart(chart, getChartProgress(chart), reduced));
...
/* =====================================================================
   v124 — exact rail parity engine.
   This intentionally does only one thing: it makes the long-run treasury
   answer rail use the same viewport-progress timing as the chapter seam line
   beside “03 Outcome quality.” It runs after prior handlers so it can correct
   older accumulated rail patches without touching card choreography, chart
   drawing, gate behavior, or the homepage scroll engine.
   ===================================================================== */
(function(){
  const chapterSeams = Array.from(document.querySelectorAll('.chapter-seam'));
  const calls = Array.from(document.querySelectorAll('.outcome-call-with-line'));
  if (!chapterSeams.length && !calls.length) return;
  ...
  const ensureCallRail = (call) => {
    ...
    if (!rail) {
      rail = document.createElement('span');
      rail.className = 'outcome-call-rail';
      rail.setAttribute('aria-hidden', 'true');
      call.appendChild(rail);
    }
    let fill = rail.querySelector('.outcome-call-rail-fill');
    if (!fill) {
      fill = document.createElement('span');
      fill.className = 'outcome-call-rail-fill';
      rail.appendChild(fill);
    }
    let beam = rail.querySelector('.outcome-call-rail-beam');
    if (!beam) {
      beam = document.createElement('span');
      beam.className = 'outcome-call-rail-beam';
      beam.setAttribute('aria-hidden', 'true');
      rail.appendChild(beam);
    }
    return { rail, fill, beam };
  };

  const rails = calls.map((call) => ({ call, ...ensureCallRail(call) }));

  const lineProgressFrom = (line) => {
    if (!line) return 0;
    const rect = line.getBoundingClientRect();
    const height = Math.max(1, window.innerHeight || document.documentElement.clientHeight || 1);
    const y = rect.top + rect.height * 0.5;
    // Same timing window for chapter seams and long-run answer rails:
    // begin as the rail enters the lower reading field, complete while it is
    // still visibly centered in the customer's viewport.
    const start = height * 0.88;
    const end = height * 0.56;
    return clamp01((start - y) / Math.max(1, start - end));
  };

  const seamProgressFromLine = (line) => {
    if (reduce && reduce.matches) return 1;
    const p = lineProgressFrom(line);
    return clamp01(0.14 + p * 0.86);
  };
  ...
  const paint = () => {
    ...
    chapterSeams.forEach((seam) => {
      const line = seam.querySelector('.chapter-seam-line') || seam;
      const seamP = seamProgressFromLine(line);
      seam.style.setProperty('--chapter-seam-progress', seamP.toFixed(4));
    });

    rails.forEach(({ call, rail, fill, beam }) => {
      const seamP = seamProgressFromLine(rail);
      call.style.setProperty('--outcome-call-line-progress', seamP.toFixed(4));
      call.style.setProperty('--outcome-call-seam-progress', seamP.toFixed(4));
      setImportant(fill, 'transform', `scaleX(${seamP})`);
      setImportant(fill, 'opacity', String(0.18 + seamP * 0.78));
      setImportant(beam, 'transform', `translate3d(${-120 + seamP * 220}%,0,0) skewX(-18deg)`);
      setImportant(beam, 'opacity', String(0.10 + seamP * 0.62));
    });
  };
  ...
  if (document.fonts && document.fonts.ready) document.fonts.ready.then(request).catch(() => {});
  if (reduce && reduce.addEventListener) reduce.addEventListener('change', request);
  [0,120,360,900].forEach((delay) => window.setTimeout(request, delay));
  request();
})();

/* =====================================================================
   v126 — exact rail parity repair.
   The 03 Outcome quality chapter seam and the long-run treasury answer rail
   are driven by the same viewport-progress function and painted with the
   same fill/beam transforms. This runs after all historical handlers so it
   restores the 03 seam and keeps the long-run rail identical on desktop and
   mobile. No card, chart, gate, or page scroll choreography is changed.
   ===================================================================== */
(function(){
  const reduce = window.matchMedia ? window.matchMedia('(prefers-reduced-motion: reduce)') : null;
  const clamp01 = (value) => Math.max(0, Math.min(1, Number.isFinite(value) ? value : 0));
  ...
  const ensureLongRunRail = () => {
    const call = document.querySelector('.outcome-call-with-line');
    if (!call) return null;
    let rail = call.querySelector('.outcome-call-rail');
    if (!rail) {
      rail = document.createElement('span');
    ...
    rail.classList.add('chapter-seam-line');
    let fill = rail.querySelector('.outcome-call-rail-fill, .chapter-seam-fill');
    if (!fill) {
      fill = document.createElement('span');
      fill.className = 'outcome-call-rail-fill chapter-seam-fill';
      rail.appendChild(fill);
    }
    fill.classList.add('outcome-call-rail-fill', 'chapter-seam-fill');
    let beam = rail.querySelector('.outcome-call-rail-beam, .chapter-seam-beam');
    if (!beam) {
      beam = document.createElement('span');
      beam.className = 'outcome-call-rail-beam chapter-seam-beam';
      beam.setAttribute('aria-hidden', 'true');
    ...
    beam.classList.add('outcome-call-rail-beam', 'chapter-seam-beam');
    return { owner: call, rail, fill, beam };
  };

  const getRails = () => {
    const proofSeam = document.querySelector('#proof .proof-shell > .chapter-seam.chapter-seam-light');
    const proofLine = proofSeam ? proofSeam.querySelector('.chapter-seam-line') : null;
    const proofFill = proofSeam ? proofSeam.querySelector('.chapter-seam-fill') : null;
    const proofBeam = proofSeam ? proofSeam.querySelector('.chapter-seam-beam') : null;
    const rails = [];
    if (proofSeam && proofLine && proofFill && proofBeam) {
      rails.push({ owner: proofSeam, rail: proofLine, fill: proofFill, beam: proofBeam });
    }
    const longRun = ensureLongRunRail();
    if (longRun) rails.push(longRun);
    return rails;
  };
  ...
  const progressForRail = (rail) => {
    ...
    // Same timing as the original chapter seam: starts in the lower reading
    // field and completes while the rail is still visibly on screen.
    const start = viewport * 0.88;
    const end = viewport * 0.56;
    const raw = (start - centerY) / Math.max(1, start - end);
    return clamp01(0.14 + clamp01(raw) * 0.86);
  };

  const paintRail = ({ owner, rail, fill, beam }, reduced) => {
    const p = reduced ? 1 : progressForRail(rail);
    if (owner) {
      owner.style.setProperty('--chapter-seam-progress', p.toFixed(4));
      owner.style.setProperty('--outcome-call-seam-progress', p.toFixed(4));
      owner.style.setProperty('--outcome-call-line-progress', p.toFixed(4));
      owner.style.setProperty('--rs-exact-seam-progress', p.toFixed(4));
    }
  ...
  const paint = () => {
    ...
    getRails().forEach((entry) => paintRail(entry, reduced));
  };
  const request = () => {
    if (ticking) return;
    ticking = true;
...
(function(){
  ...
  const paint = () => {
    ...
    targets.forEach(({root,line,fill,beam}) => {
      const raw = reduced ? 1 : progressFromLine(line);
      const p = clamp01(0.14 + raw * 0.86);
      root.style.setProperty('--section-progress', raw.toFixed(4));
      root.style.setProperty('--section-focus', clamp01(raw * 1.35).toFixed(4));
      root.style.setProperty('--chapter-seam-progress', p.toFixed(4));
      root.style.setProperty('--rs-live-seam-progress', p.toFixed(4));
      root.style.setProperty('--outcome-call-line-progress', p.toFixed(4));
      root.style.setProperty('--outcome-call-seam-progress', p.toFixed(4));
      setImportant(fill, 'transform', `scaleX(${p})`);
      setImportant(fill, 'opacity', String(0.18 + p * 0.78));
      setImportant(beam, 'transform', `translate3d(${-120 + p * 220}%,0,0) skewX(-18deg)`);
      setImportant(beam, 'opacity', String(0.10 + p * 0.62));
...
/* =====================================================================
   v129 — true desktop rail scroll parity.
   Prior rail sweeps used percent transforms, which are relative to the beam's
   own width. That reads on short mobile rails but barely moves on long desktop
   rails. This scoped final pass computes beam travel in actual rail pixels and
   paints after the older handlers.
   ===================================================================== */
(function(){
  const clamp01 = (value) => Math.max(0, Math.min(1, Number.isFinite(value) ? value : 0));
  ...
  const ensureLongRunRail = () => {
    const root = document.querySelector('.outcome-call-with-line');
    if (!root) return null;
    let line = root.querySelector('.outcome-call-rail');
    if (!line) {
      line = document.createElement('span');
      line.className = 'outcome-call-rail chapter-seam-line';
      line.setAttribute('aria-hidden', 'true');
      root.appendChild(line);
    }
    line.classList.add('outcome-call-rail', 'chapter-seam-line');
    let fill = line.querySelector('.outcome-call-rail-fill');
    if (!fill) {
      fill = document.createElement('span');
      fill.className = 'outcome-call-rail-fill chapter-seam-fill';
    ...
    fill.classList.add('outcome-call-rail-fill', 'chapter-seam-fill');
    let beam = line.querySelector('.outcome-call-rail-beam');
    if (!beam) {
      beam = document.createElement('span');
      beam.className = 'outcome-call-rail-beam chapter-seam-beam';
      beam.setAttribute('aria-hidden', 'true');
    ...
    beam.classList.add('outcome-call-rail-beam', 'chapter-seam-beam');
    return { root, line, fill, beam };
  };

  const collectTargets = () => {
    const targets = [];
    const longRun = ensureLongRunRail();
    if (longRun) targets.push(longRun);
    const proofRoot = document.querySelector('#proof .chapter-seam-light');
    if (proofRoot) {
      const line = proofRoot.querySelector('.chapter-seam-line');
      const fill = proofRoot.querySelector('.chapter-seam-fill');
      const beam = proofRoot.querySelector('.chapter-seam-beam');
      if (line && fill && beam) targets.push({ root: proofRoot, line, fill, beam });
    }
    return targets;
  };
  ...
  const paintTarget = (target) => {
    if (!target || !target.root || !target.line || !target.fill || !target.beam) return;
    const rect = target.line.getBoundingClientRect();
    if (!rect.width || !rect.height) return;
    const raw = rawProgressForLine(target.line);
    const progress = clamp01(0.14 + raw * 0.86);
    const progressText = progress.toFixed(4);
    const rawText = raw.toFixed(4);
    const focusText = clamp01(raw * 1.35).toFixed(4);
    const railWidth = Math.max(1, rect.width);
    const isDesktop = window.innerWidth >= 761;
    const beamWidth = Math.round(Math.max(isDesktop ? 96 : 48, Math.min(isDesktop ? 260 : 132, railWidth * 0.24)));
    const beamX = Math.round(-beamWidth + raw * (railWidth + beamWidth));

    target.root.style.setProperty('--section-progress', rawText, 'important');
    target.root.style.setProperty('--section-focus', focusText, 'important');
    target.root.style.setProperty('--chapter-seam-progress', progressText, 'important');
    target.root.style.setProperty('--rs-live-seam-progress', progressText, 'important');
    target.root.style.setProperty('--rs-exact-seam-progress', progressText, 'important');
    target.root.style.setProperty('--outcome-call-line-progress', progressText, 'important');
    target.root.style.setProperty('--outcome-call-seam-progress', progressText, 'important');
    target.root.style.setProperty('--rs-rail-progress', progressText, 'important');
    target.line.style.setProperty('--rs-rail-progress', progressText, 'important');
    target.line.style.setProperty('--rs-rail-beam-width', `${beamWidth}px`, 'important');
    target.line.style.setProperty('--rs-rail-beam-x', `${beamX}px`, 'important');

    setImportant(target.fill, 'transform', `scaleX(${progress})`);
    setImportant(target.fill, 'opacity', String(0.18 + progress * 0.78));
    setImportant(target.fill, 'transform-origin', 'left center');
    setImportant(target.beam, 'left', '0');
    setImportant(target.beam, 'right', 'auto');
    setImportant(target.beam, 'width', `${beamWidth}px`);
    setImportant(target.beam, 'height', '1px');
    setImportant(target.beam, 'top', '0');
    setImportant(target.beam, 'bottom', 'auto');
    setImportant(target.beam, 'filter', 'none');
    setImportant(target.beam, 'transform', `translate3d(${beamX}px,0,0) skewX(-18deg)`);
    setImportant(target.beam, 'opacity', String(0.16 + progress * 0.68));
  };

  const refreshTargets = () => { targets = collectTargets(); };
  const paint = () => {
    if (!targets.length) refreshTargets();
  ...
  const settlePaint = () => {
    ...
    settleFrames -= 1;
    if (settleFrames > 0) {
      window.requestAnimationFrame(settlePaint);
    } else {
      settleRunning = false;
    }
  };
  const requestPaint = (frames = 12) => {
    // Paint immediately for responsive scroll feedback, then keep painting for
    // a few frames so older queued rail handlers cannot reclaim the transform.
    paint();
  ...
  const refreshAndPaint = (frames = 18) => { refreshTargets(); requestPaint(frames); };

  window.addEventListener('scroll', () => requestPaint(14), { passive:true });
  window.addEventListener('resize', () => refreshAndPaint(20));
  window.addEventListener('orientationchange', () => window.setTimeout(() => refreshAndPaint(22), 120));
  window.addEventListener('load', () => refreshAndPaint(24), { once:true });
  window.addEventListener('pageshow', () => refreshAndPaint(24));
  document.addEventListener('rs:gate-unlocked', () => refreshAndPaint(24));
  if (window.visualViewport) window.visualViewport.addEventListener('resize', () => refreshAndPaint(20), { passive:true });
  if (document.fonts && document.fonts.ready) document.fonts.ready.then(() => refreshAndPaint(24)).catch(() => {});
  [0,80,220,520,1000,1600].forEach((delay) => window.setTimeout(() => refreshAndPaint(24), delay));
  refreshAndPaint(24);
})();
