(function(){
  ...
  // v155: mobile edge-overscroll hardening. At the document edges, some mobile
  // browsers expose a white native canvas/pull-to-refresh surface. Prevent only
  // that edge pull; preserve normal scrolling, nested scroll regions, forms,
  // and all approved scroll-linked animation behavior.
  (function setupRSMobileEdgeOverscrollGuard(){
    ...
    window.addEventListener('touchmove', (event) => {
      if (!tracking || !event.touches || event.touches.length !== 1 || event.defaultPrevented) return;
  ...
  document.addEventListener('click', (event) => {
    ...
    if ((hrefFile === 'index.html' || hrefFile === '') && getRSScrollY() > 2) {
...
document.addEventListener('rs:gate-unlocked', function(){
  window.requestAnimationFrame(function(){
    try {
      // Do not synthesize a global scroll event on unlock. The specific visual
      // engines already subscribe to rs:gate-unlocked/pageshow. Avoiding a full
      // scroll cascade prevents mobile browsers from feeling like they reloaded.
...
// v155 publish-readiness audit: v149/v153 visual baseline preserved; mobile edge-overscroll hardening finalized.
