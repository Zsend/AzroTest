(function(){
  ...
  const whyNowRail = document.querySelector('#why-now .copy-sticky-rail');
  const whyNowTargetCard = document.querySelector('#why-now [data-sequence-card="3"]');
  ...
  let whyNowStartStickY = 0;
  let whyNowMaxTravel = 0;

  const updateWhyNowRail = () => {
    if (!whyNowRail || !whyNowCopy || !whyNowTargetCard) return;
    if (window.innerWidth < 1180) {
      whyNowRail.style.removeProperty('--copy-rail-height');
      whyNowCopy.style.removeProperty('--copy-travel-y');
      whyNowStartStickY = 0;
      whyNowMaxTravel = 0;
      return;
    }

    whyNowCopy.style.setProperty('--copy-travel-y', '0px');
    whyNowRail.style.removeProperty('--copy-rail-height');

    const headerHeight = parseFloat(getComputedStyle(doc).getPropertyValue('--header-height')) || 84;
    const stickyTop = headerHeight + 28;
    const railTop = getDocTop(whyNowRail);
    const copyTop = getDocTop(whyNowCopy);
    const copyHeight = whyNowCopy.offsetHeight;
    const targetBottom = getDocTop(whyNowTargetCard) + whyNowTargetCard.offsetHeight;

    whyNowMaxTravel = Math.max(0, Math.round(targetBottom - (copyTop + copyHeight) + 1));
    whyNowStartStickY = Math.round(copyTop - stickyTop);

    const railHeight = Math.max(copyHeight, Math.round((copyTop - railTop) + copyHeight + whyNowMaxTravel));
    whyNowRail.style.setProperty('--copy-rail-height', `${railHeight}px`);
  };
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      if (section.id === 'why-now') {
        ...
        if (whyNowCopy) {
          ...
          if (window.innerWidth >= 1180 && !prefersReducedMotion()) {
            const travel = clamp(window.scrollY - whyNowStartStickY, 0, whyNowMaxTravel);
            whyNowCopy.style.setProperty('--copy-travel-y', `${travel.toFixed(2)}px`);
          } else {
            whyNowCopy.style.setProperty('--copy-travel-y', '0px');
          }
        }
      ...
      if (section.id === 'process') {
        const processProgress = ramp(progress, 0.00, 0.34);
        const processSweep = ramp(progress, 0.00, 0.30);
        ...
        processCards.forEach((card, idx) => {
          const stepProgress = ramp(progress, idx * 0.06, 0.14);
  ...
  const refreshResponsiveMeasures = () => {
    updateWhyNowRail();
    requestScrollFx();
  };

  window.addEventListener('scroll', requestScrollFx, { passive: true });
  window.addEventListener('resize', refreshResponsiveMeasures);
  window.addEventListener('load', refreshResponsiveMeasures);
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(refreshResponsiveMeasures).catch(() => {});
  }
  if (reduceMotionQuery.addEventListener) {
    reduceMotionQuery.addEventListener('change', refreshResponsiveMeasures);
  }
  updateWhyNowRail();
  requestScrollFx();
})();
