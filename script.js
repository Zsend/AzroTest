(function(){
  ...
  // v120: mobile stability guard, intentionally non-invasive.
  ...
  const RS_SCROLL_STATE_KEY = 'rs_scroll_state_v120';
  ...
  // v120 customer-ready build — last-known-good scroll engine restored — refined hero scale, precision section anchoring, replayable downward card reveals, final color polish, and production launch fixes.
  ...
  // v120: chart motion is owned by the final chart scrub engine at the end of this file.
...
/* v120: seam and outcome-call progress calibration.
...
/* =====================================================================
   v120 — proof chart + long-run line scroll engine, rebuilt cleanly.

   This is the only engine that owns the SVG chart draw and the rule under
   “The long-run treasury answer…”. It intentionally uses the chart's own
   viewport position on desktop and mobile, so the desktop chart now grows the
   same way it does on mobile: as the chart is actually entering the reading
   field, not from the sticky section's parent geometry.
   ===================================================================== */
(function(){
  ...
  const chartProgress = (chart, vh) => {
    ...
    // Same mental model on desktop and mobile: the chart begins drawing as it
    // enters the lower reading field and finishes after it reaches the upper
    // reading field. This is what the earlier mobile treatment did well.
    const isDesktop = window.innerWidth >= 1024;
    const start = vh * (isDesktop ? 0.90 : 0.92);
    const end = vh * (isDesktop ? 0.20 : 0.14);
    return clamp01((start - rect.top) / Math.max(1, start - end));
  };

  const paintChart = (chart, progress, reduced) => {
    const p = reduced ? 1 : clamp01(progress);
    const endpoint = reduced ? 1 : ease((p - 0.86) / 0.14);
    ...
    chart.querySelectorAll('[data-line]').forEach((path) => {
      const len = parseFloat(path.dataset.lineLength || '1') || 1;
      path.style.strokeDasharray = String(len);
      path.style.strokeDashoffset = String((1 - p) * len);
    });

    chart.querySelectorAll('[data-chart-area]').forEach((path) => {
      const areaClip = `inset(0 ${(1 - p) * 100}% 0 0)`;
      const areaOpacity = String(0.10 + p * 0.90);
      path.style.clipPath = areaClip;
      path.style.opacity = areaOpacity;
      path.style.setProperty('--chart-area-clip', areaClip);
      path.style.setProperty('--chart-area-opacity', areaOpacity);
    });

    chart.querySelectorAll('[data-endpoint]').forEach((circle) => {
      const scale = String(0.72 + endpoint * 0.28);
      circle.style.opacity = String(endpoint);
      circle.style.transform = `scale(${scale})`;
      circle.style.setProperty('--chart-endpoint-opacity', String(endpoint));
      circle.style.setProperty('--chart-endpoint-scale', scale);
    });
  };

  const callLineProgress = (call, vh) => {
    const rect = call.getBoundingClientRect();

    // Match the chapter seam language: the rule starts when the text enters the
    // lower reading field and completes before the text leaves the comfortable
    // desktop/mobile reading zone.
    const start = vh * 0.90;
    const end = vh * 0.50;
    return clamp01((start - rect.top) / Math.max(1, start - end));
  };
  ...
  const update = () => {
    ...
    charts.forEach((chart) => paintChart(chart, chartProgress(chart, vh), reduced));
    calls.forEach((call) => {
      const p = reduced ? 1 : clamp01(callLineProgress(call, vh));
  ...
  if (document.fonts && document.fonts.ready) document.fonts.ready.then(request).catch(() => {});
  if (reduce && reduce.addEventListener) reduce.addEventListener('change', request);
  window.setTimeout(request, 80);
  window.setTimeout(request, 260);
  request();
})();

/* =====================================================================
   v120 — final chart + long-run rail correction.
   Desktop chart timing now uses the proof rail's scroll window because the
   chart sits inside a sticky panel; mobile keeps the chart-entering-viewport
   model. This is appended last so older chart handlers cannot win.
   ===================================================================== */
(function(){
  ...
  const ensureChartPrepared = (chart) => {
    const svg = chart.querySelector('svg');
    if (!svg) return;
    svg.querySelectorAll('path[stroke^="url("]').forEach((path) => {
      path.setAttribute('data-line', '');
      let len = Number(path.dataset.lineLength || 0);
      if (!len) {
        try { len = Math.max(1, path.getTotalLength()); } catch (e) { len = 1; }
        path.dataset.lineLength = String(len);
      }
      path.style.strokeDasharray = String(len);
    });
  ...
  charts.forEach(ensureChartPrepared);

  const desktopChartProgress = (chart, vh) => {
    const grid = chart.closest('.proof-sticky-grid') || document.querySelector('#proof .proof-grid');
    if (!grid) return null;
    const rect = grid.getBoundingClientRect();
    const gridTop = rect.top + window.scrollY;
    const gridBottom = rect.bottom + window.scrollY;
    const start = gridTop - (vh * 0.33);
    const end = gridBottom - (vh * 0.70);
    return clamp01((window.scrollY - start) / Math.max(1, end - start));
  };

  const mobileChartProgress = (chart, vh) => {
    const rect = chart.getBoundingClientRect();
    const start = vh * 0.92;
    const end = vh * 0.14;
    return clamp01((start - rect.top) / Math.max(1, start - end));
  ...
  const chartProgress = (chart, vh) => {
    if (window.innerWidth >= 1024) {
      const desktop = desktopChartProgress(chart, vh);
      if (desktop !== null) return desktop;
    }
    return mobileChartProgress(chart, vh);
  };
  ...
  const paintChart = (chart, progress, reduced) => {
    ...
    chart.querySelectorAll('[data-chart-area]').forEach((path) => {
      ...
      const opacity = String(0.10 + p * 0.90);
      path.style.clipPath = clip;
  ...
  const update = () => {
    ...
    charts.forEach((chart) => paintChart(chart, chartProgress(chart, vh), reduced));
    calls.forEach((call) => {
      const p = reduced ? 1 : lineProgress(call, vh);
      call.style.setProperty('--outcome-call-line-progress', p.toFixed(4));
    });
  };
