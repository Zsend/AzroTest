    ...
    const updateCards = () => {
      if (prefersReducedMotion()) {
        slideInBandCards.forEach((card) => {
          ...
          card.style.setProperty('will-change', 'auto');
        });
        return;
      }
    ...
    const updateClosingScrollCards = () => {
      if (prefersReducedMotion()) {
        closingScrollCards.forEach((card) => {
          ...
          card.style.setProperty('filter', 'none', 'important');
          card.style.setProperty('will-change', 'auto');
          card.classList.add('has-scroll-landed');
          card.classList.remove('is-cta-focus-window', 'is-scroll-paused');
        });
        return;
      }
      ...
      closingScrollCards.forEach((card, idx) => {
        ...
        const progress = progressFromTopPx(top, timing.startY, timing.endY);
