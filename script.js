(function(){
  ...
  if (slideInBandCards.length) {
    ...
    // many pixels, so card 1 starts animating 140px of scroll AFTER
    // card 0 starts, card 2 starts 280px after card 0. Gives the trio
    // a clear Asset → Problem → System cadence.
    const SCROLL_STAGGER = 140;

    // Progress = 0 when the card's effective top crosses 88% of the
    // viewport height (just entering from the bottom).
    // Progress = 1 when the effective top reaches 28% of the viewport
    // height (well into the upper-middle area). Range ≈ 0.6 × viewport
    // height — typically 540px on a 900px viewport, so a card takes
    // about half a screen of scroll to fully reveal. Slow + deliberate.
    const START_RATIO = 0.88;
    const END_RATIO = 0.28;
