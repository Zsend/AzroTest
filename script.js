(function(){
  ...
  const createSectionTransition = (fromItem, toItem) => {
    const scene = document.createElement('section');
    scene.className = 'section-transition';
    scene.setAttribute('aria-hidden', 'true');
    scene.dataset.from = fromItem.id;
    scene.dataset.to = toItem.id;
    scene.innerHTML = `
      <div class="section-transition__sticky">
        <div class="section-transition__grid"></div>
        <div class="section-transition__wash"></div>
        <div class="section-transition__aura section-transition__aura--a"></div>
        <div class="section-transition__aura section-transition__aura--b"></div>
        <div class="container section-transition__inner">
          <div class="section-transition__pane section-transition__pane--from">
            <div class="section-transition__eyebrow">Leaving chapter</div>
            <div class="section-transition__num">${fromItem.number || ''}</div>
            <div class="section-transition__label">${fromItem.label || fromItem.id}</div>
            <div class="section-transition__title">${fromItem.title || ''}</div>
          </div>
          <div class="section-transition__seam">
            <span class="section-transition__line section-transition__line--base"></span>
            <span class="section-transition__line section-transition__line--track"></span>
            <span class="section-transition__beam"></span>
            <span class="section-transition__core"></span>
          </div>
          <div class="section-transition__pane section-transition__pane--to">
            <div class="section-transition__eyebrow">Entering chapter</div>
            <div class="section-transition__num">${toItem.number || ''}</div>
            <div class="section-transition__label">${toItem.label || toItem.id}</div>
            <div class="section-transition__title">${toItem.title || ''}</div>
          </div>
        </div>
      </div>
    `;
  ...
  if (sectionsById.length > 1) {
    sectionsById.forEach((item, index) => {
      const next = sectionsById[index + 1];
      if (!next) return;
      const scene = createSectionTransition(item, next);
      item.el.insertAdjacentElement('afterend', scene);
    });
  }

  const transitionScenes = [...document.querySelectorAll('.section-transition')];
  ...
  const updateScrollFx = () => {
    ...
    transitionScenes.forEach((scene, index) => {
      ...
      scene.style.setProperty('--transition-progress', progress.toFixed(3));
      scene.style.setProperty('--transition-pulse', pulse.toFixed(3));
      scene.style.setProperty('--transition-focus', focus.toFixed(3));
      const isActive = rect.top < window.innerHeight * 0.72 && rect.bottom > window.innerHeight * 0.28;
      scene.classList.toggle('is-active', isActive);
      scene.classList.toggle('is-past', progress >= 0.995);
      scene.classList.toggle('is-upcoming', progress <= 0.005);
    });
