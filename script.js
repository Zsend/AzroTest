...
(() => {
  ...
  const toggleGalleryFullscreen = async () => {
    ...
    if (!enteredNative && activeSet === 'video') {
  ...
  roots.forEach((root) => {
    ...
    const controller = {
      ...
      render(modeOverride) {
        ...
        launch.setAttribute('aria-label', modeKey === 'video' ? `Play ${this.data.title || 'preview videos'}` : `Open ${this.data.title || 'preview images'} fullscreen`);
        ...
        if (modeKey === 'image') {
          ...
          expandBadge.className = 'media-gallery__expand-badge';
          expandBadge.setAttribute('aria-hidden', 'true');
          expandBadge.textContent = 'Expand';
          launch.appendChild(expandBadge);
        }
