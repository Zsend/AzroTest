  ...
  const updateScrollFx = () => {
    ...
    document.querySelectorAll('.section[id]').forEach((section, index) => {
      ...
      if (section.id === 'process') {
        // v14: line must visibly traverse all 4 cards. Sweep starts a touch
        // before the first card animates and finishes just after the last,
        // so the eye reads it as "the line draws while cards light up under it."
        const processProgress = ramp(progress, 0.06, 0.46);
        const processSweep = ramp(progress, 0.06, 0.46);
        ...
        processCards.forEach((card, idx) => {
          const stepProgress = ramp(progress, idx * 0.10 + 0.08, 0.22);
