(function(){
  ...
  revealNodes.forEach((node, index) => {
    // The Asset / Problem / System framework cards have their own
    // scroll-linked entrance below. Keep them out of the generic reveal
    // observer so the original scroll-load behavior cannot be flattened
    // into a one-time fade.
    if (node.matches && node.matches('.band-grid .band-card')) return;
  ...
  if (slideInBandCards.length) {
    ...
    const updateCards = () => {
      ...
      const scrollStagger = compact ? 68 : SCROLL_STAGGER;
      const startRatio = compact ? 1.06 : START_RATIO;
      const endRatio = compact ? 0.70 : END_RATIO;
      ...
      slideInBandCards.forEach((card, idx) => {
        ...
        // Desktop preserves the original horizontal editorial slide.
        // Stacked/mobile layouts use a vertical lift so the motion remains
        // scroll-linked without creating side-to-side overflow.
        const tx = reduced ? 0 : (compact ? 0 : (1 - e) * 140);
        const ty = reduced ? 0 : (compact ? (1 - e) * 24 : 0);
        const sc = reduced ? 1 : (compact ? 0.982 + e * 0.018 : 0.96 + e * 0.04);
        const bl = reduced ? 0 : (compact ? (1 - e) * 5 : (1 - e) * 10);
        const opacity = reduced ? (0.58 + e * 0.42) : e;
