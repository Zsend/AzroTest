...
(() => {
  ...
  const tryInlinePreviewPlay = (video) => {
    if (!video || !document.contains(video) || document.hidden) return;
    video.muted = true;
  ...
  const retryInlinePreviewVideos = () => {
    document.querySelectorAll('.media-gallery__preview-video').forEach((video) => {
      tryInlinePreviewPlay(video);
    });
  };

  if (!window.__azroPreviewRetryBound) {
    window.__azroPreviewRetryBound = true;
    window.addEventListener('load', retryInlinePreviewVideos);
    window.addEventListener('pageshow', retryInlinePreviewVideos);
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) retryInlinePreviewVideos();
    });
  }
      ...
      render(modeOverride) {
        ...
        if (modeKey === 'video') {
          ...
          if (allowPreviewAutoplay) {
            const video = document.createElement('video');
            const previewSrc = current.preview || current.src;
            ...
            const clearReady = () => visualWrap.classList.remove('is-video-ready');
            video.addEventListener('playing', markReady, { once: true });
            video.addEventListener('timeupdate', markReady, { once: true });
            video.addEventListener('loadedmetadata', () => tryInlinePreviewPlay(video), { once: true });
            video.addEventListener('loadeddata', () => tryInlinePreviewPlay(video), { once: true });
            video.addEventListener('canplay', () => tryInlinePreviewPlay(video), { once: true });
            video.addEventListener('error', clearReady);
            visualWrap.appendChild(video);
            video.load();
            requestAnimationFrame(() => tryInlinePreviewPlay(video));
            setTimeout(() => tryInlinePreviewPlay(video), 160);
          }
