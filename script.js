...
(() => {
  ...
  imageNodes.forEach((img) => {
    ...
    trigger.setAttribute('aria-label', 'View ' + (img.alt || 'image'));
...
/* Footer email capture (FormSubmit AJAX with native fallback) */
(() => {
  ...
  if (!lightbox) {
    ...
    lightbox.innerHTML = `
      ...
      <div class="modal__dialog modal__dialog--gallery" role="dialog" aria-modal="true" aria-label="Preview gallery">
        <button class="modal__close gallery-lightbox__fullscreen" data-gallery-fullscreen-toggle type="button" aria-label="Open full screen" title="Open full screen" hidden>
          <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M8 3H4v4"></path><path d="M16 3h4v4"></path><path d="M8 21H4v-4"></path><path d="M16 21h4v-4"></path><path d="M4 8 9 3"></path><path d="M15 3 20 8"></path><path d="M4 16 9 21"></path><path d="M15 21 20 16"></path></svg>
  ...
  roots.forEach((root) => {
    ...
    const controller = {
      ...
      render(modeOverride) {
        ...
        launch.setAttribute('aria-label', modeKey === 'video' ? `View ${this.data.title || 'preview videos'}` : `View ${this.data.title || 'preview images'}`);
...
/* Proof-card layout lock: when any proof docs card is open, stack the proof cards so one open card doesn't leave dead space beside the other. */
(() => {
...
/* Final publish v222: pricing QR fullscreen shares the existing image lightbox and exposes a mobile control. */
(() => {
  const qrImages = Array.from(document.querySelectorAll('.crypto-address-qr'));
  ...
  const modalImg = modal.querySelector('.image-lightbox__figure img');
  const caption = modal.querySelector('.image-lightbox__caption');
  const closeEls = Array.from(modal.querySelectorAll('[data-image-close]'));
  ...
  const close = () => {
    ...
    if (modalImg) {
      modalImg.removeAttribute('src');
      modalImg.removeAttribute('data-qr-fullscreen');
    }
    if (caption) {
      caption.textContent = '';
    }
    if (modal._azroLastFocus && typeof modal._azroLastFocus.focus === 'function') {
      modal._azroLastFocus.focus();
    }
  };

  if (modal.dataset.azroQrFullscreenBound !== '1') {
    closeEls.forEach((el) => {
    ...
    modal.dataset.azroQrFullscreenBound = '1';
  }

  const open = (img) => {
    if (!img || !modalImg || !caption) return;
    modal._azroLastFocus = document.activeElement;
    modalImg.src = img.currentSrc || img.src;
    modalImg.alt = img.alt || '';
    modalImg.setAttribute('data-qr-fullscreen', '1');

    const card = img.closest('.crypto-address-card');
    const label = card?.querySelector('.crypto-address-label')?.textContent?.trim() || '';
    const help = card?.querySelector('.crypto-address-help')?.textContent?.trim() || '';
    const text = [label, help].filter(Boolean).join(' — ') || img.alt || 'QR code';
    caption.textContent = text;
  ...
  qrImages.forEach((img) => {
    if (img.dataset.azroQrFullscreenBound === '1') return;
    img.dataset.azroQrFullscreenBound = '1';
    img.setAttribute('tabindex', '0');
    img.setAttribute('role', 'button');
    img.setAttribute('aria-label', (img.alt || 'Open QR code') + ' full screen');

    const wrap = img.closest('.crypto-address-qr-wrap');
    if (wrap && !wrap.querySelector('.media-fullscreen-btn')) {
      const fsBtn = document.createElement('button');
      fsBtn.type = 'button';
      fsBtn.className = 'media-fullscreen-btn';
      fsBtn.innerHTML = '<span aria-hidden="true">⤢</span><span class="sr-only">Open QR code full screen</span>';
      fsBtn.setAttribute('aria-label', 'Open QR code full screen');
...
/* Final publish v224: explicit full-screen control inside the gallery lightbox for both videos and images.
   Uses native fullscreen when supported and falls back to an immersive full-viewport mode on mobile/touch. */
(() => {
  const lightbox = document.getElementById('galleryLightbox');
  if (!lightbox) return;

  const dialog = lightbox.querySelector('.modal__dialog--gallery');
  const closeBtn = lightbox.querySelector('.modal__close[data-gallery-close]');
  const stage = lightbox.querySelector('[data-gallery-lightbox-stage]');
  if (!dialog || !closeBtn || !stage) return;

  let fsBtn = lightbox.querySelector('[data-gallery-fullscreen-toggle]');
  if (!fsBtn) {
    fsBtn = document.createElement('button');
    fsBtn.type = 'button';
    fsBtn.className = 'modal__close gallery-lightbox__fullscreen';
    fsBtn.setAttribute('data-gallery-fullscreen-toggle', '');
    fsBtn.hidden = true;
    closeBtn.insertAdjacentElement('beforebegin', fsBtn);
  }

  const expandIcon = '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M8 3H4v4"></path><path d="M16 3h4v4"></path><path d="M8 21H4v-4"></path><path d="M16 21h4v-4"></path><path d="M4 8 9 3"></path><path d="M15 3 20 8"></path><path d="M4 16 9 21"></path><path d="M15 21 20 16"></path></svg>';
  const collapseIcon = '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M9 9H4V4"></path><path d="M15 9h5V4"></path><path d="M9 15H4v5"></path><path d="M15 15h5v5"></path><path d="M4 4 9 9"></path><path d="M20 4 15 9"></path><path d="M4 20 9 15"></path><path d="M20 20 15 15"></path></svg>';

  const getMedia = () => stage.querySelector('video, img');
  const getVideo = () => stage.querySelector('video');
  ...
  const isDialogFullscreen = () => {
    ...
    return !!(fsEl && (fsEl === dialog || dialog.contains(fsEl)));
  };
  ...
  const isNativeVideoFullscreen = () => {
    ...
    try {
      if (video.webkitDisplayingFullscreen === true) return true;
    } catch (_) {}
    try {
  ...
  const isImmersive = () => lightbox.classList.contains('is-immersive');
  const isFullscreenActive = () => isDialogFullscreen() || isNativeVideoFullscreen() || isImmersive();
  ...
  const setButtonState = (active) => {
    ...
    fsBtn.innerHTML = (active ? collapseIcon : expandIcon) + '<span class="sr-only">' + label + '</span>';
  };

  const syncButton = () => {
    const visible = lightbox.classList.contains('is-open') && !!getMedia();
    fsBtn.hidden = !visible;
    fsBtn.setAttribute('aria-hidden', visible ? 'false' : 'true');
    setButtonState(isFullscreenActive());
  };
  ...
  const exitElementFullscreen = async () => {
    ...
    try {
      if (typeof document.msExitFullscreen === 'function' && document.msFullscreenElement) {
  ...
  const enterFullscreen = async () => {
    if (!lightbox.classList.contains('is-open') || isFullscreenActive()) return;

    const video = getVideo();
    if (video && lightbox.classList.contains('is-video-mode')) {
      const openedVideoFullscreen = await requestVideoFullscreen(video);
      if (openedVideoFullscreen) {
        window.setTimeout(syncButton, 40);
        return;
      }
    }

    const openedDialogFullscreen = await requestElementFullscreen(dialog);
    if (!openedDialogFullscreen) {
      lightbox.classList.add('is-immersive');
    }
    syncButton();
  };
  ...
  Array.from(lightbox.querySelectorAll('[data-gallery-close]')).forEach((node) => {
  ...
  document.addEventListener('fullscreenchange', () => window.requestAnimationFrame(syncButton));
  document.addEventListener('webkitfullscreenchange', () => window.requestAnimationFrame(syncButton));
  document.addEventListener('msfullscreenchange', () => window.requestAnimationFrame(syncButton));
  window.addEventListener('resize', syncButton, { passive: true });
  window.addEventListener('orientationchange', syncButton, { passive: true });

  const bindVideoFullscreenEvents = () => {
    const video = getVideo();
    if (!video || video.dataset.azroGalleryFsBound === '1') return;
    video.dataset.azroGalleryFsBound = '1';
    video.addEventListener('webkitbeginfullscreen', syncButton);
  ...
  lightboxObserver.observe(lightbox, { attributes: true, attributeFilter: ['class', 'aria-hidden'] });
...
/* Final publish v225: give the standalone image lightbox the same fullscreen behavior as videos.
   Uses native fullscreen when supported and falls back to an immersive full-viewport mode. */
(() => {
  const modal = document.getElementById('imageLightbox');
  if (!modal) return;

  const dialog = modal.querySelector('.modal__dialog');
  const closeBtn = modal.querySelector('.modal__close[data-image-close]');
  const figure = modal.querySelector('.image-lightbox__figure');
  if (!dialog || !closeBtn || !figure) return;

  let fsBtn = modal.querySelector('[data-image-fullscreen-toggle]');
  if (!fsBtn) {
    fsBtn = document.createElement('button');
    fsBtn.type = 'button';
    fsBtn.className = 'modal__close image-lightbox__fullscreen';
    fsBtn.setAttribute('data-image-fullscreen-toggle', '');
    fsBtn.hidden = true;
    closeBtn.insertAdjacentElement('beforebegin', fsBtn);
  }
  ...
  const getImage = () => modal.querySelector('.image-lightbox__figure img[src], .image-lightbox__figure img[data-qr-fullscreen="1"]');
  const getFullscreenElement = () => document.fullscreenElement || document.webkitFullscreenElement || document.msFullscreenElement || null;
  ...
  const isImmersive = () => modal.classList.contains('is-immersive');
  const isFullscreenActive = () => isDialogFullscreen() || isImmersive();
  ...
  const syncButton = () => {
    const visible = modal.classList.contains('is-open') && !!getImage();
    fsBtn.hidden = !visible;
    fsBtn.setAttribute('aria-hidden', visible ? 'false' : 'true');
    setButtonState(isFullscreenActive());
  };
  ...
  const enterFullscreen = async () => {
    if (!modal.classList.contains('is-open') || isFullscreenActive()) return;
    const openedDialogFullscreen = await requestElementFullscreen(dialog);
    if (!openedDialogFullscreen) {
  ...
  Array.from(modal.querySelectorAll('[data-image-close]')).forEach((node) => {
    node.addEventListener('click', () => {
      modal.classList.remove('is-immersive');
      exitElementFullscreen();
      syncButton();
  ...
  const figureObserver = new MutationObserver(syncButton);
  figureObserver.observe(figure, { childList: true, subtree: true, attributes: true, attributeFilter: ['src', 'data-qr-fullscreen'] });
