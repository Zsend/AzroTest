...
(() => {
  ...
  const getPreviewVideoPreloadHost = () => {
    let host = document.getElementById('gallery-preview-preload-host');
    if (host) return host;
    host = document.createElement('div');
    host.id = 'gallery-preview-preload-host';
  ...
  roots.forEach((root) => {
    ...
    const controller = {
      ...
      render(modeOverride) {
        ...
        if (typeof root._galleryPreviewCleanup === 'function') {
          try { root._galleryPreviewCleanup(); } catch (_) {}
          root._galleryPreviewCleanup = null;
        }
        ...
        if (modeKey === 'video') {
          ...
          if (allowPreviewAutoplay) {
            const preloadHost = getPreviewVideoPreloadHost();
            const video = document.createElement('video');
            ...
            let mountedForPlayback = false;
            let disposed = false;

            const markReady = () => {
              if (previewReady || disposed) return;
              previewReady = true;
            ...
            const cleanupPreview = () => {
              ...
              if (root._galleryPreviewCleanup === cleanupPreview) {
                root._galleryPreviewCleanup = null;
              }
            };
            root._galleryPreviewCleanup = cleanupPreview;
            const mountForPlayback = () => {
              if (disposed || mountedForPlayback || !frameDecoded) return;
              mountedForPlayback = true;
              if (video.parentNode !== visualWrap) {
                visualWrap.appendChild(video);
              }
            };
            const maybeReveal = () => {
              if (disposed) return;
              if (frameDecoded) mountForPlayback();
              if (frameDecoded && playHasStarted) markReady();
            };
            const tryInlinePlay = () => {
              if (disposed || document.hidden) return;
              mountForPlayback();
              if (!mountedForPlayback) return;
              const playPromise = video.play();
            ...
            const retryInlinePlay = () => {
              if (disposed) return;
              if (video.readyState >= 2) frameDecoded = true;
              maybeReveal();
              tryInlinePlay();
            };
