(function(){
  ...
  // v114: mobile stability guard. iOS/Brave can occasionally repaint or reload
  // memory-heavy long pages while the customer is mid-scroll. The site must not
  // feel like it randomly restarted at the top. Store the current scroll
  // position cheaply and restore it only for same-page reload/back-forward
  // recoveries. Fresh first visits still start normally.
  const RS_SCROLL_STATE_KEY = 'rs_scroll_state_v114';
  ...
  const saveRSScrollState = () => {
    try {
      ...
      if (y < 24) return;
      sessionStorage.setItem(RS_SCROLL_STATE_KEY, JSON.stringify({
        path: currentPathForRestore(),
        y,
        t: Date.now()
      }));
    } catch (e) { /* ignore */ }
  };
  let rsScrollSaveTick = false;
  window.addEventListener('scroll', () => {
    if (rsScrollSaveTick) return;
    rsScrollSaveTick = true;
    window.setTimeout(() => {
      rsScrollSaveTick = false;
      saveRSScrollState();
    }, 240);
  }, { passive: true });
  window.addEventListener('pagehide', saveRSScrollState, { passive: true });
  const restoreRSScrollState = (reason) => {
    if (window.location.hash) return;
    try {
      const raw = sessionStorage.getItem(RS_SCROLL_STATE_KEY);
      if (!raw) return;
      const state = JSON.parse(raw);
      if (!state || state.path !== currentPathForRestore()) return;
      if (!Number.isFinite(state.y) || state.y < 220) return;
      if (Date.now() - (state.t || 0) > 30 * 60 * 1000) return;
      const kind = navigationKind();
      if (!(reason === 'pageshow-persisted' || kind === 'reload' || kind === 'back_forward')) return;
      window.requestAnimationFrame(() => {
        const maxY = Math.max(0, document.documentElement.scrollHeight - window.innerHeight);
        window.scrollTo({ top: Math.min(state.y, maxY), left: 0, behavior: 'auto' });
        window.requestAnimationFrame(() => window.dispatchEvent(new Event('scroll')));
      });
    } catch (e) { /* ignore */ }
  };
  window.addEventListener('pageshow', (event) => restoreRSScrollState(event.persisted ? 'pageshow-persisted' : 'pageshow'), { once: true });
  window.addEventListener('load', () => restoreRSScrollState('load'), { once: true });

  // v114 customer-ready build — last-known-good scroll engine restored — refined hero scale, precision section anchoring, replayable downward card reveals, final color polish, and production launch fixes.
