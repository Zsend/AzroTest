...
(() => {
  ...
  // Generic grouped details behavior (used for top-level BTC/XRP cards)
  const groupedDetails = Array.from(document.querySelectorAll('details[data-accordion-group]'));
  if (groupedDetails.length) {
    groupedDetails.forEach((d) => {
      d.addEventListener('toggle', () => {
        if (!d.open) return;
        const group = d.getAttribute('data-accordion-group');
        groupedDetails.forEach((other) => {
          if (other !== d && other.getAttribute('data-accordion-group') === group) other.open = false;
        });
      });
    });
  }
