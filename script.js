    ...
    if (videoBuyEl) {
      ...
      if (poster) {
        ...
        video.removeAttribute('poster');
			...
			video.muted = true;
			video.loop = !['demo', 'full'].includes(videoMode);
			if (typeof syncVideoControls === 'function') syncVideoControls();
...
(() => {
  ...
  const resumeInlinePreviewVideos = (scope = document) => {
    if (document.hidden) return;
    const rootNode = scope && typeof scope.querySelectorAll === 'function' ? scope : document;
    rootNode.querySelectorAll('.media-gallery__preview-video').forEach((video) => {
      tryInlinePreviewPlay(video);
    });
  };

  window.addEventListener('load', () => {
    requestAnimationFrame(() => resumeInlinePreviewVideos(document));
  });
  ...
  roots.forEach((root) => {
    ...
    const controller = {
      ...
      render(modeOverride) {
        ...
        if (modeKey === 'video') {
          ...
          const poster = document.createElement('img');
          const previewSrc = current.previewSrc || current.src;
          const posterSrc = current.previewPoster || current.poster || (previewSrc && /\.mp4(?:[?#].*)?$/i.test(previewSrc)
            ? previewSrc.replace(/\.mp4((?:[?#].*)?)$/i, '-poster.webp$1')
            : previewSrc);
    ...
    controller.render();
    requestAnimationFrame(() => resumeInlinePreviewVideos(root));
  });
})();
