(function(){
  ...
  // ---------------------------------------------------------------------
  // v70 — Closing-card scroll scrub with CTA-weighted timing.
  //
  // Operator Materials keeps the same scroll timing as v69. Next Step uses
  // the same motion language but a longer scroll window and later finish so
  // the Book a Bitcoin Treasury Review card becomes the deliberate visual
  // landing point instead of appearing at the same speed as the document card.
  // This remains scroll-position-driven: no one-time reveal class, no CSS
  // transition driving the entrance, and no layout changes.
  // ---------------------------------------------------------------------
  ...
  if (closingScrollCards.length) {
    closingScrollCards.forEach((card) => {
      card.removeAttribute('data-reveal');
      card.classList.remove('is-visible', 'rs-scroll-slide-card', 'is-landed', 'has-scroll-landed', 'is-cta-focus-window');
      card.style.setProperty('will-change', 'transform, opacity, filter');
      card.style.setProperty('opacity', '0', 'important');
      card.style.setProperty('transform', 'translate3d(140px, 0, 0) scale(0.9600)', 'important');
      card.style.setProperty('filter', 'blur(10px)', 'important');
    });

    const easeOutCubicClosing = (t) => 1 - Math.pow(1 - t, 3);
    const CLOSING_START_RATIO = 1.10;
    const CLOSING_END_RATIO = 0.62;
    const CLOSING_CARD_TIMING = [
      // Operator Materials: unchanged from v69.
      { offset: 0, duration: 1, drift: 140, blur: 10, scaleFrom: 0.96, ease: easeOutCubicClosing },
      // Next Step: same entrance, slower scroll window, later completion.
      { offset: 132, duration: 1.72, drift: 148, blur: 10, scaleFrom: 0.958, ease: easeOutCubicClosing }
    ];

    const updateClosingScrollCards = () => {
      const viewportH = window.innerHeight || document.documentElement.clientHeight || 800;
      const startY = viewportH * CLOSING_START_RATIO;
      const endY = viewportH * CLOSING_END_RATIO;
      const baseRange = Math.max(1, startY - endY);

      closingScrollCards.forEach((card, idx) => {
        const timing = CLOSING_CARD_TIMING[idx] || CLOSING_CARD_TIMING[CLOSING_CARD_TIMING.length - 1];
        const rect = card.getBoundingClientRect();
        const effectiveTop = rect.top + timing.offset;
        const range = baseRange * timing.duration;
        const raw = (startY - effectiveTop) / range;
        const progress = Math.max(0, Math.min(1, raw));
        const e = timing.ease(progress);
        ...
        card.classList.toggle('is-cta-focus-window', idx === 1 && progress > 0.18 && progress < 0.995);
      });
    };
