...
/* Final publish v223: add an explicit full-screen control inside the gallery lightbox on mobile.
   Uses the native Fullscreen API when available and falls back to an immersive full-viewport mode. */
(() => {
  ...
  const switchWrap = lightbox.querySelector('[data-gallery-lightbox-switch]');
  ...
  if (!fsBtn) {
    ...
    fsBtn.innerHTML = '<span class="gallery-lightbox__fullscreen-icon" aria-hidden="true">⤢</span><span class="sr-only">Enter full screen</span>';
  ...
  const isNativeDialogFullscreen = () => {
  ...
  const isNativeVideoFullscreen = () => {
    ...
    try {
      return !!(video && video.webkitDisplayingFullscreen === true);
    } catch (_) {
      return false;
    }
  };
  const isImmersive = () => lightbox.classList.contains('is-immersive');
  const isFullscreenActive = () => isNativeDialogFullscreen() || isNativeVideoFullscreen() || isImmersive();
  ...
  const setButtonState = (active) => {
    ...
    fsBtn.innerHTML = '<span class="gallery-lightbox__fullscreen-icon" aria-hidden="true">' + icon + '</span><span class="sr-only">' + label + '</span>';
  };

  const syncButton = () => {
    const visible = isMobileViewport() && lightbox.classList.contains('is-open');
    fsBtn.hidden = !visible;
    fsBtn.setAttribute('aria-hidden', visible ? 'false' : 'true');
    setButtonState(isFullscreenActive());

    if (switchWrap && visible) {
      switchWrap.style.setProperty('--azro-gallery-actions', '2');
    }
  };
  ...
  const enterFullscreen = async () => {
    ...
    const requested = await requestElementFullscreen(dialog);
    if (!requested) {
