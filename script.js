...
/* =====================================================================
   v127 — final rail parity engine.
   Purpose: restore the moving rail beside “03 Outcome quality” and give the
   long-run treasury answer rail the same visual structure and same viewport-
   scrubbed behavior on desktop and mobile. This is the single owner of those
   two rails. It does not touch card choreography, chart drawing, gate logic,
   or the page-level progress rail.
   ===================================================================== */
(function(){
  ...
  const ensureLongRunRail = () => {
    ...
    rail.classList.add('rs-live-chapter-rail');
    fill.classList.add('rs-live-chapter-rail-fill');
    beam.classList.add('rs-live-chapter-rail-beam');
  ...
  const buildTargets = () => {
    ...
    if (proofSeam) {
      const line = proofSeam.querySelector('.chapter-seam-line');
      const fill = proofSeam.querySelector('.chapter-seam-fill');
      const beam = proofSeam.querySelector('.chapter-seam-beam');
      if (line && fill && beam) {
        line.classList.add('rs-live-chapter-rail');
        fill.classList.add('rs-live-chapter-rail-fill');
        beam.classList.add('rs-live-chapter-rail-beam');
        targets.push({ root: proofSeam, line, fill, beam });
      }
    }
  ...
  const progressForLine = (line) => {
    ...
    // Shared rail timing: begin as the rail enters the lower reading field,
    // finish while it is still visibly in the central reading band. The wider
    // window makes the motion readable on desktop and mobile instead of
    // snapping or appearing already complete.
    const start = viewport * 0.92;
    const end = viewport * 0.38;
    return clamp01((start - y) / Math.max(1, start - end));
  };

  const paintTarget = ({ root, line, fill, beam }, reduced) => {
    const raw = reduced ? 1 : progressForLine(line);
    const p = reduced ? 1 : clamp01(0.14 + raw * 0.86);
    const focus = clamp01(raw * 1.35);

    root.style.setProperty('--section-progress', raw.toFixed(4));
    root.style.setProperty('--section-focus', focus.toFixed(4));
  ...
  const paint = () => {
    ...
    targets.forEach((target) => paintTarget(target, reduced));
  };
