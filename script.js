(function(){
  ...
  // v116: mobile stability guard, intentionally non-invasive.
  ...
  const RS_SCROLL_STATE_KEY = 'rs_scroll_state_v116';
  ...
  // v116 customer-ready build — last-known-good scroll engine restored — refined hero scale, precision section anchoring, replayable downward card reveals, final color polish, and production launch fixes.
  ...
  // v116: Scroll-scrubbed proof chart animation.
  // The prior IntersectionObserver flipped the chart to 100% as soon as it
  // intersected, which made the line feel like it snapped/gitched into place
  // on mobile and did not read as a true scroll graphic on desktop. This keeps
  // the animation tied to viewport progress, with a long enough travel window
  // to feel deliberate and institutional instead of flashy.
  ...
  const updateProofChartMotion = () => {
    if (!proofCharts.length) return;

    if (prefersReducedMotion()) {
      proofCharts.forEach(chart => {
        chart.style.setProperty('--chart-ready', '1');
        chart.style.setProperty('--chart-sweep', '1');
        chart.classList.add('is-animated');
      });
      return;
    }

    const vh = Math.max(1, window.innerHeight || doc.clientHeight || 1);
    const proofRailForChart = document.querySelector('#proof .proof-rail');
    proofCharts.forEach(chart => {
      let raw = 0;
      let sweepRaw = 0;

      // Desktop proof chart lives inside the sticky proof lens. Its own
      // bounding box clamps while sticky, so using chart.getBoundingClientRect()
      // makes the line jump from 0 to nearly complete. Use the proof rail
      // position instead so the chart grows across the full sticky reading
      // window, matching the section's editorial pacing.
      if (window.innerWidth > 1200 && proofRailForChart && chart.closest('#proof')) {
        const railTop = proofRailForChart.getBoundingClientRect().top;
        const start = vh * 0.64;
        const end = -vh * 1.45;
        raw = clamp((start - railTop) / Math.max(1, start - end));
        sweepRaw = clamp((start - railTop) / Math.max(1, start - (-vh * 1.65)));
      } else {
        const rect = chart.getBoundingClientRect();
        const start = vh * 0.92;
        const end = vh * (window.innerWidth <= 760 ? 0.12 : 0.20);
        raw = clamp((start - rect.top) / Math.max(1, start - end));
        sweepRaw = clamp((start - rect.top) / Math.max(1, start - vh * 0.02));
      }

      const draw = smoothStep(raw);
      const sweep = smoothStep(sweepRaw);

      chart.classList.add('is-scroll-animated');
      chart.style.setProperty('--chart-ready', draw.toFixed(4));
      chart.style.setProperty('--chart-sweep', sweep.toFixed(4));
    });
  };
...
/* v116: seam and outcome-call progress calibration.
   Uses each element's own viewport position so lines finish while visible on
   both mobile and desktop, independent of long section heights. */
(function(){
  const seams = Array.from(document.querySelectorAll('.chapter-seam'));
  const outcomeCalls = Array.from(document.querySelectorAll('.outcome-call-with-line'));
  if (!seams.length && !outcomeCalls.length) return;
  ...
  const update = () => {
    ...
    seams.forEach((seam) => {
      ...
      seam.style.setProperty('--chapter-seam-progress', progress.toFixed(3));
    });
    outcomeCalls.forEach((call) => {
      ...
      const end = vh * 0.58;
      const progress = clamp01((start - rect.top) / Math.max(1, start - end));
      call.style.setProperty('--outcome-call-line-progress', progress.toFixed(3));
    });
  };
  const request = () => {
    if (ticking) return;
...
/* v116: scroll-driven proof chart and long-run outcome line.
   Runs after the stable homepage engine and overrides only CSS custom
   properties on the proof chart and the single outcome-call line. This avoids
   touching the tuned card choreography while making chart growth slower and
   available on desktop and mobile. */
(function(){
  const charts = Array.from(document.querySelectorAll('.path-chart'));
  ...
  const chartProgress = (chart, vh) => {
    const proof = chart.closest('#proof') || chart.closest('.section') || chart;
    const proofRail = proof && proof.querySelector ? proof.querySelector('.proof-rail') : null;

    if (window.innerWidth > 1200 && proofRail) {
      const railTop = proofRail.getBoundingClientRect().top;
      // Desktop proof chart sits inside a sticky panel; its own rect clamps,
      // so use the proof rail travel. This keeps the chart drawing throughout
      // the same sticky reading window as the Outcome Quality story cards.
      const start = vh * 0.64;
      const end = -vh * 1.45;
      return clamp01((start - railTop) / Math.max(1, start - end));
    }

    const rect = chart.getBoundingClientRect();
    // Mobile/tablet: use the chart's own visible position. The 0.82vh window is
    // intentionally long so the plot grows with the scroll instead of flashing.
    return clamp01((vh * 0.92 - rect.top) / Math.max(1, vh * 0.82));
  };
  ...
  const update = () => {
    ...
    charts.forEach((chart) => {
      const p = reduced ? 1 : ease(chartProgress(chart, vh));
      const sweep = reduced ? 1 : clamp01((p - 0.06) / 0.90);
      chart.style.setProperty('--chart-ready', p.toFixed(4));
      chart.style.setProperty('--chart-sweep', sweep.toFixed(4));
      chart.classList.toggle('is-animated', p > 0.001);
    });

    calls.forEach((call) => {
      if (reduced) {
        call.style.setProperty('--outcome-call-line-progress', '1');
        return;
      }
      const rect = call.getBoundingClientRect();
      const start = vh * 0.86;
      ...
      call.style.setProperty('--outcome-call-line-progress', ease(p).toFixed(4));
    });
  };
...
/* =====================================================================
   v117 — final proof chart scrub + outcome-call rule.
   Runs last and owns only SVG chart draw variables/styles plus the single
   long-run outcome callout rule. This prevents older one-shot chart handlers
   or decorative sweep layers from changing the approved homepage choreography.
   ===================================================================== */
(function(){
  const charts = Array.from(document.querySelectorAll('.path-chart'));
  const calls = Array.from(document.querySelectorAll('.outcome-call-with-line'));
  ...
  const setupChart = (chart) => {
    ...
    svg.querySelectorAll('path[stroke^="url("]').forEach((path) => {
      path.setAttribute('data-line', '');
      let len = 1;
    ...
    svg.querySelectorAll('path[fill^="url("]').forEach((path) => {
  ...
  const chartProgress = (chart, vh) => {
    if (window.innerWidth > 1200) {
      const rail = chart.closest('#proof')?.querySelector('.proof-rail') || chart.closest('#proof') || chart;
  ...
  const paintChart = (chart, progress) => {
    const p = smooth(progress);
    const endpoint = smooth((p - 0.72) / 0.28);
    chart.style.setProperty('--chart-ready', p.toFixed(4));
    chart.style.setProperty('--chart-sweep', p.toFixed(4));
    chart.classList.toggle('is-animated', p > 0.001);

    chart.querySelectorAll('[data-line]').forEach((path) => {
      const len = parseFloat(path.dataset.lineLength || path.style.getPropertyValue('--line-length') || '1') || 1;
      path.style.strokeDasharray = String(len);
      path.style.strokeDashoffset = String((1 - p) * len);
    });

    chart.querySelectorAll('[data-chart-area]').forEach((path) => {
      path.style.clipPath = `inset(0 ${(1 - p) * 100}% 0 0)`;
      path.style.opacity = String(0.10 + p * 0.90);
    });

    chart.querySelectorAll('[data-endpoint]').forEach((circle) => {
      circle.style.opacity = String(endpoint);
      circle.style.transform = `scale(${0.72 + endpoint * 0.28})`;
    });
  };

  const callProgress = (call, vh) => {
    const rect = call.getBoundingClientRect();
    const start = vh * 0.88;
    const end = vh * (window.innerWidth > 1200 ? 0.54 : 0.50);
    return clamp01((start - rect.top) / Math.max(1, start - end));
  };
  ...
  const update = () => {
    ...
    charts.forEach((chart) => {
      const progress = reduced ? 1 : chartProgress(chart, vh);
      paintChart(chart, progress);
    });

    calls.forEach((call) => {
      const p = reduced ? 1 : smooth(callProgress(call, vh));
