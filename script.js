(function () {
  ...
  form.addEventListener('submit', (event) => {
    ...
    const lines = [
      'Reserve Standard Bitcoin Reserve Program application',
      '',
      `Name: ${name}`,
      `Business: ${business}`,
      `Email: ${email}`,
      ...
      '',
      'Submitted from reservestandard.com static Reserve Review form.'
    ];

    const subject = encodeURIComponent(`Reserve Review — ${business}`);
    const body = encodeURIComponent(lines.join('\n'));
    const emailTarget = form.getAttribute('data-contact-email') || 'hello@reservestandard.com';
    const href = `mailto:${emailTarget}?subject=${subject}&body=${body}`;

    window.location.href = href;
    status.textContent = 'Your email app should open with a pre-filled application. If nothing happens, email hello@reservestandard.com directly.';
