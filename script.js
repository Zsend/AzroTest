...
(function(){
  ...
  const paintChart = (chart, raw, reduced) => {
    // v148: the Benchmark Lens chart is one-way per viewport visit. It never
  ...
  const chartVisitProgress = (chart, allowGrowth) => {
    const rect = chart.getBoundingClientRect();
    const height = vh();
    const fullyOffscreen = rect.bottom <= 0 || rect.top >= height;
    if (fullyOffscreen) {
    ...
    if (!allowGrowth) return Number(chart.dataset.rsMaxChartProgress || 0) || 0;
    return getChartProgress(chart);
  };
  const update = () => {
    ticking = false;
    const currentScrollY = window.scrollY || 0;
    const allowChartGrowth = currentScrollY >= lastScrollY - 1;
    const reduced = !!(reduceQuery && reduceQuery.matches);
    charts.forEach((chart) => paintChart(chart, chartVisitProgress(chart, allowChartGrowth), reduced));
