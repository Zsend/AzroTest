...
(function(){
  ...
  // Recompute CTA metrics when any dialog opens/closes (iOS/Safari landscape quirks)
  function bindDialogReflow(){
    var dialogs = document.querySelectorAll('dialog');
    dialogs.forEach(function(dlg){
      if (!dlg.__ctaBound){
        dlg.addEventListener('close', function(){ setTimeout(updateCTAPad, 0); }, {passive:true});
        dlg.addEventListener('cancel', function(){ setTimeout(updateCTAPad, 0); }, {passive:true});
        dlg.__ctaBound = true;
      }
    });
  }
  bindDialogReflow();
  // If modals are injected dynamically, rebind
  var moDialogs = new MutationObserver(function(){ bindDialogReflow(); updateCTAPad(); });
  moDialogs.observe(document.documentElement, {subtree:true, childList:true});
