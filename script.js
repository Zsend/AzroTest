...
(() => {
  ...
  if (!lightbox) {
    ...
    lightbox.innerHTML = `
      ...
      <div class="modal__dialog modal__dialog--gallery" role="dialog" aria-modal="true" aria-labelledby="galleryLightboxTitle">
        <h2 id="galleryLightboxTitle" class="sr-only">TradingView preview</h2>
  ...
  const renderLightbox = () => {
    ...
    if (lbTitle) lbTitle.textContent = current.label || activeController.data.title || 'TradingView preview';
    if (lbCaption) lbCaption.textContent = current.label || activeController.data.caption || 'Preview the media.';
