(function(){
  ...
  // ---------------------------------------------------------------------
  // v75 — Closing-card scroll choreography with a guaranteed early landing.
  //
  // The closing cards use the same scroll-scrub language as the Asset /
  // Problem / System cards. The key launch requirement: by the time the
  // Operator Materials / Next Step row reaches the natural reading position
  // under the sticky header, both cards must already be fully sharp, opaque,
  // and landed. Operator Materials enters deliberately, then a small scroll
  // pause holds the eye before Next Step slides in as the obvious action card.
  // ---------------------------------------------------------------------
  ...
  if (closingScrollCards.length) {
    ...
    // Desktop choreography is based on the shared row top. Values are viewport
    // ratios for the row's top edge. Scrolling down makes rect.top smaller:
    //   • Operator starts just below the viewport and lands slowly by 82vh.
    //   • The quiet scroll pause runs from 82vh to 76vh.
    //   • Next Step starts after that pause and lands by 64vh — well before
    //     the card row reaches the sticky-header reading zone. This prevents
    //     the screenshot state where the Next Step card is still blurred while
    //     the section is already in view.
    const ROW_MOTION = [
      { startRatio: 1.22, endRatio: 0.82, drift: 144, blur: 10, scaleFrom: 0.958 },
      { startRatio: 0.76, endRatio: 0.64, drift: 146, blur: 10, scaleFrom: 0.958 }
    ];

    // Stacked responsive choreography: each card uses its own top edge so the
    // second card does not reveal before the customer reaches it. The natural
    // vertical space between cards supplies the pause on mobile/tablet, and
    // each card lands before it becomes the primary reading object.
    const STACKED_MOTION = [
      { startRatio: 1.14, endRatio: 0.74, drift: 84, blur: 9, scaleFrom: 0.962 },
      { startRatio: 1.08, endRatio: 0.72, drift: 82, blur: 9, scaleFrom: 0.962 }
    ];

    const progressFromTop = (top, startRatio, endRatio, viewportH) => {
      const startY = viewportH * startRatio;
      const endY = viewportH * endRatio;
      return clamp01((startY - top) / Math.max(1, startY - endY));
    };

    const paintClosingCard = (card, progress, timing) => {
      const e = easeOutCubicClosing(progress);
    ...
    const updateClosingScrollCards = () => {
      ...
      const firstRect = closingScrollCards[0].getBoundingClientRect();
      const secondRect = closingScrollCards[1] ? closingScrollCards[1].getBoundingClientRect() : firstRect;
      const isStacked = Math.abs(secondRect.top - firstRect.top) > 44;

      closingScrollCards.forEach((card, idx) => {
        const timing = isStacked
          ? (STACKED_MOTION[idx] || STACKED_MOTION[STACKED_MOTION.length - 1])
          : (ROW_MOTION[idx] || ROW_MOTION[ROW_MOTION.length - 1]);
        const top = isStacked ? card.getBoundingClientRect().top : firstRect.top;
        let progress = progressFromTop(top, timing.startRatio, timing.endRatio, viewportH);

        // Arrival guard: if the row has reached the same visual zone shown in
        // the founder screenshot, both closing cards must be completely landed.
        // This protects against browser scroll inertia, smooth-scroll settling,
        // viewport-height differences, and minor layout shifts after fonts load.
        if (!isStacked && firstRect.top <= viewportH * 0.64) progress = 1;
        if (isStacked && top <= viewportH * 0.70) progress = 1;

        paintClosingCard(card, progress, timing);
        card.classList.toggle('is-scroll-paused', !isStacked && idx === 0 && firstRect.top < viewportH * 0.82 && firstRect.top > viewportH * 0.76);
        card.classList.toggle('is-cta-focus-window', idx === 1 && progress > 0.10 && progress < 0.995);
      });
    };
