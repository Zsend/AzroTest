(function () {
  ...
  function footerTargetY() {
    ...
    return Math.max(0, Math.round(footer.offsetTop - headerHeight() + 8));
  }
  ...
  function nearestPanelIndex(y = window.scrollY) {
    ...
    const viewportProbe = y + headerHeight() + Math.max(140, (window.innerHeight - headerHeight()) * 0.42);
    let winner = 0;
    let bestDistance = Number.POSITIVE_INFINITY;

    panels.forEach((panel, index) => {
      const panelMid = panel.offsetTop + panel.offsetHeight * 0.5;
      const distance = Math.abs(panelMid - viewportProbe);
  ...
  function syncActiveFromScroll(forceHash) {
    ...
    const nextIndex = footerIsVisible() ? panels.length - 1 : nearestPanelIndex();
    const nextPanel = panels[nextIndex];
    if (!nextPanel) return;
    setActivePanel(nextPanel.id, { updateHash: Boolean(forceHash) && !footerIsVisible() });
  }
  ...
  function animateTo(y, opts = {}) {
    ...
    if (Math.abs(distance) < 3) {
      ...
      isAnimating = false;
      body.classList.remove('is-chapter-animating');
      syncActiveFromScroll(Boolean(opts.updateHash));
      lockUntil = performance.now() + 180;
      return;
    }

    const duration = Math.min(980, Math.max(620, Math.abs(distance) * 0.55));
  ...
  function goToIndex(index, opts = {}) {
    ...
    if (!target) return;
    setActivePanel(target.id, { updateHash: false });
    animateTo(panelTargetY(clamped), { updateHash: opts.updateHash !== false });
  }

  function goToFooter() {
    if (!footer) return;
    animateTo(footerTargetY(), { updateHash: false });
  }
  ...
  function settleToNearest() {
    ...
    animateTo(targetY, { updateHash: true });
  }

  function scheduleSettle() {
    if (!guidedEnabled || isAnimating) return;
    clearTimeout(settleTimer);
    settleTimer = window.setTimeout(settleToNearest, 140);
  }
  ...
  function canGuideNow() {
    return desktopGuideMedia.matches && !prefersReducedMotion.matches && window.innerHeight >= 720;
  }
  ...
  function handleAnchorNavigation(event) {
    ...
    animateTo(section.offsetTop, { updateHash: true });
  }
  ...
  window.addEventListener('keydown', (event) => {
    ...
    if (performance.now() < lockUntil || isAnimating) {
  ...
  function handleResize() {
    ...
    rafResize = requestAnimationFrame(() => {
      ...
      if (guidedEnabled && !footerIsVisible()) {
        setActivePanel(panels[currentIndex()]?.id || activePanelId);
      }
    });
  }

  desktopGuideMedia.addEventListener('change', updateGuideMode);
